...
- ⬜ **PDF export still broken** — the Export button produces a blank PDF. All Phase 1 PDFs shared this session were generated via browser File → Print, not the export button. Root cause: `#reportContainer` has `.hidden` class (display:none) when no report is loaded; the print CSS `display:block !important` fix was applied but is not fully resolving the issue. Parked — fix after extractor rebuild. Do not confuse "export works" with "print works" — they are different code paths.
