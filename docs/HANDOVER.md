...
**Two-paper + data paper plan:**

1. **Paper 1 — Methodology/framework** (target: *JAMIA* or *JCE*): V4 architecture — schema-as-product framing, critic mechanism, deterministic post-processing, audit trail. Target audience: clinical informatics / health technology assessment. Key claim: forced structured schema is the durable anti-hallucination mechanism; model choice is secondary.

2. **Paper 2 — Surgical validation** (target: *BJS* or *Annals of Surgery*): Prospective blinded validation in 30 general surgery RCTs. Primary endpoints: MA field accuracy vs pre-pipeline ground truth, time saving (crossover design). Secondary endpoints: overall error rate, error taxonomy, critic utility, model agnosticism. Target audience: surgical academics and systematic review community.

3. **Data paper** (target: *Scientific Data* or *BMJ Open*): The 30-paper validated extraction library — structured, arbitrated, FAIR-compliant. Enables others to benchmark against the same ground truth.
...
- Session 19 (2026-04-27): PROTOCOL.md v2.0 finalised. Key additions: (1) PI excluded from Phase 1a entirely (pipeline designer cannot do ground truth extraction); (2) Phase 0 papers (ORBITA etc.) formally excluded from formal 30-paper set — selection bias from iterative tuning; (3) Crossover design formalised (papers 1–15: Pair A does Phase 1a, Pair B Phase 2a; papers 16–30: swap) — eliminates rater speed confound, within-rater paired comparison; (4) Phase 1a arbitration procedure for evaluative fields (when raters disagree on RoB/GRADE, arbitrator resolves to consensus before Phase 1a data lock); (5) Phase 2 rater blinding explicit (_critic trail not shown, only field values visible); (6) N=30 justification rewritten: powered for overall accuracy (~510 field observations, ±3% Wilson CI at 85%); per-field N=30 → descriptive only; (7) Secondary endpoints rewritten: subgroup analyses → descriptive breakdowns (not hypothesis-tested); model agnosticism as pre-specified secondary (run alternative extractor on all 30 papers, same schema, compare vs Phase 1a ground truth); (8) Pilot meta-analysis: arbitrated output (not raw V4), DerSimonian–Laird random effects pre-specified to match Cochrane benchmark; (9) Pipeline runtime recorded as _runtime_seconds in V4 JSON (3-line change in api/study.js); (10) Reporting framework: STARD-informed (not TRIPOD-AI primary — no single framework fits; STARD as structural reference, TRIPOD-AI + DECIDE-AI for AI-specific items); (11) Publication strategy documented: Paper 1 (methodology → JAMIA/JCE), Paper 2 (surgical validation → BJS/Annals), Data paper (validated library → Scientific Data/BMJ Open).
