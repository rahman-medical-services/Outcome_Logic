...
## Product and Publication Strategy

**Two products:**

1. **OutcomeLogic (the tool)** — on-demand extraction pipeline. Submit any PDF or DOI/PMID, receive a structured clinical evidence report in ~30 seconds. Live now. The primary product.

2. **The validated library** — a curated, growing database of human-arbitrated RCT extractions. Each entry is V4-extracted + Phase 2 human-corrected + Phase 3 arbitrated. Starts at 30 general surgery papers from the validation study; expands by specialty over time. Pre-built structured evidence that researchers and systematic reviewers can consume without re-extracting. A second product, complementary to the tool.

The two products reinforce each other: the tool generates library entries at scale; the library demonstrates what the tool produces and why it can be trusted; the paper validates both.

**One paper:**

**Validation outcomes paper** (target: *BJS*, *Annals of Surgery*, or *JAMIA*): Methods + prospective blinded validation results in 30 general surgery RCTs. Methods section covers the V4 architecture. Results cover MA field accuracy, time saving (crossover design), error taxonomy, critic utility, model agnosticism. This is the credibility anchor for both products. A methodology-only paper without results is not publishable in a target journal.
