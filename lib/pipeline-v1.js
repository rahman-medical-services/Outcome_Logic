...
8. ADVERSE EVENTS — Grade ≥3 or ≥2% either arm:
   ...
   (b) List every secondary endpoint from Section 5.
   (c) Remove from your AE candidate list ANY event matching (a) or (b) — regardless of whether it
       appears in the paper's safety table. Pre-specified endpoints belong in outcomes, never in AE.
...
9. CRITICAL APPRAISAL
   Risk of Bias — assess each Cochrane domain (randomisation, allocation concealment, blinding,
   outcome adjudication, attrition) and assign exactly one of: Low | Moderate | High.
   NEVER output "Unclear". If detail on a specific domain is missing from the text, note it in
   the rationale but still assign an overall label using available information.
   Default for an unblinded RCT with adjudicated endpoints: Moderate.

   GRADE certainty — assign exactly one of: High | Moderate | Low | Very Low.
   GRADE CALIBRATION RULE:
   - Very Low is reserved for case series, case reports, and observational studies with very
     serious methodological flaws. NEVER assign Very Low to a randomised controlled trial.
   - Low = RCT with serious limitations (e.g. no blinding, high attrition, major crossover,
     small N, single-centre).
   - Moderate = RCT with some limitations (open-label, imperfect allocation concealment,
     moderate crossover, industry funding). This is the correct default for most RCTs.
   - High = multiple consistent well-conducted RCTs with no serious limitations.

   - Key limitations stated by authors (max 2 sentences)
   - Any important industry funding or COI disclosures — name the sponsor explicitly if stated

10. CHARTS — always use recommended_chart_type: "bar"
   Populate arms[].data_points[].y with per-arm OBSERVED EVENT RATE as a raw percentage number:
   ...
   y-values come from arm_a_value / arm_b_value in primary endpoint.
