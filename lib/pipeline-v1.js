...
9. CRITICAL APPRAISAL
   ...
   NEVER output "Unclear". For each domain name it explicitly, then give an overall label.
   FALLBACK RULE: if specific domain detail is absent from the text but the paper describes a
   randomised trial, assign Moderate — do NOT output Unclear under any circumstances.
   Domain defaults when detail is absent: randomisation = Low (assumed for any RCT);
   allocation concealment = Moderate (if not explicitly described); blinding = Moderate (if
   open-label) or Low (if double-blind); outcome adjudication = Low (if independent committee
   stated) or Moderate (if unclear); attrition = Low (if <10% loss to follow-up).
