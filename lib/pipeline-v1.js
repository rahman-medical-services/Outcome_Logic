...
4. PRIMARY ENDPOINT — extract all of the following:
   ...
   - Effect measure with 95% CI — use whichever the paper reports as primary.
     CANONICAL LABELS — use exactly these abbreviations, no synonyms:
       HR  (not "hazard ratio")      OR  (not "odds ratio")
       RR  (not "risk ratio")        RD  (not "risk difference" or "rate difference")
       MD  (not "mean difference" or "difference")   SMD  (not "standardised mean difference")
     effect_measure must be one of: HR | OR | RR | RD | MD | SMD
   - P-value — extract exactly as printed. If reported as an inequality (e.g. P<0.05, P<0.001),
     extract the inequality string verbatim — do NOT convert to null. null only if not reported at all.
   ...
   - For continuous: mean (or median), SD (or IQR), and N per arm
     arm_a_sd / arm_b_sd — SCOPE RULE: only the SD of the PRIMARY ENDPOINT MEASUREMENT.
     Do NOT use SD of age, BMI, weight, or any other baseline characteristic from Table 1.
     The SD must appear in the Results section explicitly for the outcome being extracted.
     If the outcome-specific SD is not stated in the results, leave arm_a_sd / arm_b_sd null.
   - For continuous outcomes: arm_a_events and arm_b_events MUST be null. Continuous outcomes
     have no event counts. Do NOT set to 0 — zero is a valid event count for binary outcomes
     and will corrupt downstream analysis. Null means not applicable.
   ...
   PRIMARY RESULT SYNTHESIS — write 1-2 sentences combining: intervention vs comparator,
   outcome label, timeframe, point estimate with CI, and p-value. Plain English, no jargon.
   For significant results: state the direction of effect.
   For non-significant: state "did not significantly differ" or equivalent — no directional claim.
   For NI trials: state whether non-inferiority was demonstrated and cite the CI vs NI margin.
   Example (superiority): "TAVR reduced the composite of death or stroke at 2 years from 15.8% to
   8.5% (HR 0.54, 95% CI 0.37–0.79, p<0.001)."
   Example (NS): "CABG did not significantly reduce the primary composite endpoint compared to
   medical therapy over a mean 10-year follow-up (HR 0.86, 95% CI 0.72–1.04, p=0.12)."
   Store in: reportMeta.primary_result_synthesis

   UNCERTAINTY RULE — when a field is genuinely irresolvable, say so explicitly:
   If two sections of the paper give incompatible values for the same field and you cannot determine
   which is authoritative (e.g. Results text and Table 3 give different HR values with no label
   distinguishing primary from sensitivity), output null for that field AND add the field name to
   uncertain_fields on the candidate. Do NOT fabricate a value. Do NOT flip a coin.
   - uncertain_fields means: "I found conflicting information — human review needed"
   - null alone (without uncertain_fields) means: "this value is simply not reported in the paper"
   The distinction matters for Phase 0 grading: uncertain fields are pre-marked cannot_determine.
   Do NOT add to uncertain_fields for routine choices (adjusted vs unadjusted — choose adjusted;
   randomised vs analysed N — choose randomised). Only for genuinely irresolvable conflicts.
...
OUTPUT SCHEMA — output exactly this JSON structure:
{
  "reportMeta": {
    ...
    "primary_result_synthesis": "String — 1-2 sentences: intervention vs comparator, outcome, point estimate, CI, p-value, plain English. See instruction above."
  },
  "extraction_flags": {
  ...
  "primary_endpoint_candidates": [
    {
      ...
      "effect_measure": "HR | OR | RR | RD | MD | SMD | null",
