...
7. SUBGROUP ANALYSES — COMPLETENESS IS MANDATORY:
   Missing subgroups is a completeness failure equivalent to a primary endpoint extraction error.
   EXTRACTION RULE — two separate triggers:
   (a) PRE-SPECIFIED subgroups: extract ALL of them regardless of whether the interaction is significant.
       A non-significant interaction in a pre-specified subgroup is clinically informative and must be reported.
       Check the Methods, Statistical Analysis Plan, trial registration, and any forest plot figures — subgroups
       may appear in figures only (e.g. a forest plot), not in a numbered table.
   (b) POST-HOC subgroups: extract only if interaction p<0.05. Do not extract exploratory post-hoc subgroups with p≥0.05.
   If no subgroup analyses are reported in the paper, state explicitly "No subgroup analyses reported."
   COMPLETENESS CHECK: Before finalising, scan ALL figures (especially forest plots) and supplementary tables.
   If a forest plot exists, extract EVERY subgroup row shown — do not summarise or skip rows.
   - Flag each subgroup as pre-specified or post-hoc
