...
6. KAPLAN-MEIER / SURVIVAL DATA — reconstruct carefully:
   - ONLY for time-to-event endpoints (OS, DFS, PFS, etc.)
   ...
   - NOTE: If the primary outcome is a simple rate comparison (e.g. "mortality 3.7% vs 3.8%"), this is NOT a KM curve — it's a bar chart comparison
...
- Use 'stepped-line' for time-to-event / survival / Kaplan-Meier data (multiple time points)
- Use 'bar' for rates, proportions, response rates, and single-time-point comparisons (e.g. "3.7% vs 3.8%")
...
DATA FORMAT RULES:
- For stepped-line (KM curves): y-values are PROPORTIONS (0.95 means 95% survival), provide 4-6 time points
- For bar charts: y-values are PERCENTAGES as raw numbers (3.7 means 3.7%, NOT 0.037)
- If the primary outcome is a rate/proportion comparison (e.g. "death rate 3.7% vs 3.8%"), use 'bar' chart with values [3.7, 3.8]
- If insufficient data for ANY chart, output an empty endpoints array — but this should be rare for published trials

ENDPOINT EXAMPLES:

Example A — Rate comparison (BAR chart):
Trial reports: "Death due to bleeding: 3.7% in TXA vs 3.8% in placebo"
Output:
  recommended_chart_type: "bar"
  arms: [
    { group_name: "Tranexamic acid", data_points: [{ x: "5 days", y: 3.7 }] },
    { group_name: "Placebo", data_points: [{ x: "5 days", y: 3.8 }] }
  ]

Example B — Time-to-event (STEPPED-LINE chart):
Trial reports: "5-year OS: 38% vs 24%, HR 0.69"
Output:
  recommended_chart_type: "stepped-line"
  arms: [
    { group_name: "Chemotherapy", data_points: [
      { x: 0, y: 1.0 }, { x: 12, y: 0.75 }, { x: 24, y: 0.55 }, { x: 36, y: 0.45 }, { x: 48, y: 0.40 }, { x: 60, y: 0.38 }
    ]},
    { group_name: "Surgery alone", data_points: [
      { x: 0, y: 1.0 }, { x: 12, y: 0.65 }, { x: 24, y: 0.42 }, { x: 36, y: 0.32 }, { x: 48, y: 0.26 }, { x: 60, y: 0.24 }
    ]}
  ]
...
export async function runPipeline(sourceContext, sourceMeta = {}, adjMeta = {}, isEscalation = false) {
  ...
  // Model selection:
  // - Normal mode: Flash for all nodes (speed + cost efficiency)
  // - Escalation mode: Pro for all nodes (maximum accuracy on retry)
...
=======
  ...
  if (sourceMeta.sourceType === 'full-text-pdf' && !sourceMeta.pmid && !sourceMeta.doi) {
    ...
    // DOI — explicitly allow parentheses for Lancet-style DOIs e.g. S0140-6736(20)31444-6
    if (!sourceMeta.pmid) {
      const doiScan = scanText.match(/(?:DOI|doi)\s*[:\-]?\s*(10\.\d{4,}\/\S+)/i)
                   || scanText.match(/https?:\/\/doi\.org\/(10\.\d{4,}\/\S+)/i)
                   || scanText.match(/\b(10\.\d{4,}\/[^\s,;>"']{4,})/);
      if (doiScan?.[1]) {
        // Strip trailing punctuation but preserve internal parentheses
