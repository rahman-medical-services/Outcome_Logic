...
export async function runPipeline(sourceContext, sourceMeta = {}, adjMeta = {}, isEscalation = false) {
  ...
  // A and B run in parallel — saves ~10s on successful runs, giving the adjudicator
  // more headroom inside the 60s limit. With MAX_RETRIES=1, a Gemini failure throws
  // in ~2s so gpt-4o-mini barely runs; the wasted cost is negligible at this scale.
  let rawA, rawB;
  if (process.env.OPENAI_API_KEY) {
    [rawA, rawB] = await Promise.all([
      callGemini(extractModel, EXTRACTOR_PROMPT_A + escalationWarning, sourceContext, { thinkingBudget }),
