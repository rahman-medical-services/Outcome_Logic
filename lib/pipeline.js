...
5. SECONDARY ENDPOINTS — for each pre-specified secondary endpoint, extract ALL of:
   ...
   - HR or OR with 95% CI if time-to-event; RR or RD with 95% CI if binary
   - Exact p-value
   - Whether statistically significant
   Limit to the 4 most clinically important. Include the numbers — do not write vague summaries.
...
8. ADVERSE EVENTS — CRITICAL — read all sub-rules carefully:
   ...
   - INTERVENTION-ONLY AEs: if an AE is mechanistically specific to one arm (e.g. chemotherapy
     toxicities in a surgery-vs-chemo trial), set control_pct to null and note "intervention arm only"
   - Do NOT write 0% for the control arm unless the paper explicitly states zero events there
   - For observational/diagnostic studies (CPET, biomarker, risk-score studies): set has_data to
     false. Do not report surgical complication rates as Grade ≥3 AEs.
   - Treatment-related mortality and discontinuation: use null if not explicitly reported, not 0%
...
- Use 'stepped-line' for time-to-event / survival / KM data with MULTIPLE time points
- Use 'bar' for:
    (a) single fixed-timepoint binary outcomes (e.g. "death within 5 days: 3.7% vs 3.8%")
    (b) response rates, proportions, or any outcome measured at one point in time
  If the primary outcome is a single binary proportion (not a survival curve), use bar — NOT stepped-line.
- chart_priority 1 = primary endpoint
- chart_priority 2 = second chart — ONLY include if label AND data are meaningfully different
  from chart_priority 1. Do NOT output two charts with the same label or the same underlying data.
- Maximum 2 endpoints in the array
- BOTH ARMS MANDATORY: every endpoint MUST have exactly 2 arm objects. If you cannot reconstruct
  data for both arms, omit the chart entirely. A single-arm chart will render incorrectly.
- For stepped-line: 4-6 data_points per arm; y-values are proportions 0.0–1.0 (not %)
- For bar: each arm has ONE data_point: { "x": "arm label", "y": integer_percentage }
  Example: { "group_name": "TXA", "data_points": [{ "x": "TXA", "y": 4 }] }
...
- If no AE data, or observational/diagnostic study, set has_data to false and rows to []
- Set control_pct to null (not 0) when AE is mechanistically intervention-specific and not
  reported for the control arm. Add note "intervention arm only".
- Only write control_pct: 0 if the paper explicitly states zero events in the control arm
...
function postProcess(parsed, sourceMeta) {
  ...
  // Normalise AE rows — coerce undefined → null, flag all-null control
  const ae = parsed.clinician_view.adverse_events;
  if (Array.isArray(ae.rows)) {
    ae.rows = ae.rows.map(r => ({
      ...r,
      intervention_pct: r.intervention_pct ?? null,
      control_pct:      r.control_pct      ?? null,
    }));
    if (ae.rows.length && ae.rows.every(r => r.control_pct === null)) {
      ae._control_not_applicable = true;
    }
  }
  ...
  // ── Enforce max 2 charts, sorted by priority, deduplicate by label ──────────
  ...
  if (Array.isArray(endpoints)) {
    ...
    parsed.clinician_view.interactive_data.endpoints = endpoints
      ...
      .filter(ep => {
        const key = (ep.label || '').toLowerCase().trim();
