...
export async function runPipeline(sourceContext, sourceMeta = {}, adjMeta = {}, isEscalation = false) {
  ...
  async function _runExtractors(extractModel) {
    const eA = genAI.getGenerativeModel({ model: extractModel, systemInstruction: EXTRACTOR_PROMPT + escalationWarning });
    const eB = genAI.getGenerativeModel({ model: extractModel, systemInstruction: EXTRACTOR_PROMPT + escalationWarning });
