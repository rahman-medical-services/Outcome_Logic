...
5. SECONDARY ENDPOINTS — for each pre-specified secondary endpoint, extract ALL of:
   ...
   - Exact p-value and whether statistically significant
   Limit to the 4 most clinically important. Always include the numbers.
...
8. ADVERSE EVENTS — CRITICAL — read all sub-rules:
   - List each Grade ≥3 AE occurring in ≥5% of EITHER arm
   - INTERVENTION-ONLY AEs: if mechanistically specific to one arm (e.g. chemo toxicities
     in a surgery-vs-chemo trial), set control_pct to null, note "intervention arm only"
   - Do NOT write 0% for control unless the paper explicitly states zero events there
   - Observational/diagnostic studies (CPET, biomarker): set has_data false
   - Treatment-related mortality and discontinuation: use null if not explicitly reported
...
- Use 'bar' for single fixed-timepoint binary outcomes (e.g. "death within 5 days: 3.7% vs 3.8%"),
  response rates, or any proportion measured at one point in time
...
- chart_priority 2 = a clinically distinct secondary endpoint (e.g. VTE rate when primary is mortality).
  Only include if label AND data are meaningfully different from chart 1.
  Do NOT output two charts with the same label or same underlying data.
...
- If insufficient data, output an empty endpoints array

CRITICAL INSTRUCTIONS FOR ADVERSE EVENTS:
- Only include Grade ≥3 AEs occurring in ≥5% of either arm
- If no AE data, or this is an observational/diagnostic study, set has_data false and rows []
