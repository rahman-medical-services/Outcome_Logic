...
//
// GEMINI API ARCHITECTURE (April 2026)
// ────────────────────────────────────
// Uses raw fetch() to the v1beta REST endpoint. NO SDK.
//
// Why no SDK:
//   - @google/generative-ai (old): ignores thinkingConfig entirely, so
//     gemini-2.5-flash runs unbounded thinking → 503 timeout on every call.
//   - @google/genai (new): sends thinkingConfig correctly, but the SDK's
//     request construction itself triggers 503s that raw fetch() does not.
//   - Confirmed by curl: identical payloads succeed via raw REST, fail via SDK.
//
// Why gemini-2.5-flash-lite (not gemini-2.5-flash):
//   - gemini-2.5-flash has persistent 503 "high demand" errors, even with
//     thinkingBudget capping. Happens ~50% of requests — not transient.
//   - flash-lite has identical token limits (1M in / 65K out) and works reliably.
//   - Quality difference will be evaluated in Phase 0 validation study.
//
// Why thinkingBudget: 512:
//   - gemini-2.5-flash-lite minimum is 512 (128 returns 400 INVALID_ARGUMENT).
//   - gemini-2.5-flash minimum is 1 but thinkingBudget:0 returns 503.
//   - Unbounded thinking burns thousands of tokens per call, exhausting TPM quota.
//   - 512 is the minimum that satisfies all models and minimises TPM consumption.
//
// Why 5 retries with backoff:
//   - Even with flash-lite + budget 512, occasional 503s still occur (~10-20%).
//   - 5 retries with exponential backoff + jitter gives ~99% success rate.
//   - Sequential extractors (not parallel) — parallel calls 503 due to
//     per-second concurrency limits on the API key.

const GEMINI_API_KEY = process.env.GEMINI_API_KEY;
const GEMINI_BASE    = 'https://generativelanguage.googleapis.com/v1beta/models';

// Raw fetch wrapper — retries up to 5 times on 503 with exponential backoff + jitter.
// thinkingBudget: 512 caps thinking tokens to reduce TPM consumption.
async function callGemini(model, { systemInstruction, contents, temperature = 0, responseMimeType } = {}) {
  const url  = `${GEMINI_BASE}/${model}:generateContent?key=${GEMINI_API_KEY}`;
  const body = { contents };
  if (systemInstruction) {
    body.systemInstruction = { parts: [{ text: systemInstruction }] };
  }
  body.generationConfig = { temperature, thinkingConfig: { thinkingBudget: 512 } };
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (res.ok) {
      ...
      console.log(`[Gemini] Success on attempt ${attempt}/${MAX_RETRIES}`);
      return text;
    }
    ...
    if (res.status === 503 && attempt < MAX_RETRIES) {
      ...
      console.warn(`[Gemini] 503 on attempt ${attempt}/${MAX_RETRIES} — retrying in ${(delay / 1000).toFixed(1)}s`);
...
const GEMINI_MODEL_PRO     = 'gemini-2.5-flash';     // escalation uses full Flash
...
// ADJUDICATOR PROMPT
// ─────────────────────────────────────────────
const ADJUDICATOR_PROMPT_BASE = `You are the Chief of Surgery, EBM expert, and adversarial reviewer.

ADVERSARIAL REVIEW — complete this before generating JSON:

For each field where Report A and Report B AGREE:
1. Do both provide source citations? If neither does, set suspicious_agreement: true for that field.
2. Do both cite the SAME paper location? If yes, agreement is expected — corroborated.
3. Do both cite DIFFERENT locations that happen to give the same value? Ask: is there an alternative value elsewhere in the paper that neither extracted?
4. Flag suspicious_agreement: true if: values are identical with no citations; or agreement is on a field with known reporting ambiguity (e.g. adjusted vs unadjusted, ITT vs PP, abstract vs Results).

For each field where Report A and Report B DISAGREE:
1. Prefer the value backed by the more specific source citation.
2. If both cite different locations (e.g. A cites Table 2, B cites Abstract), set source_conflict: true and note both values in source_conflict_note.
3. Otherwise: favour more specific and numerically precise values.

Set ambiguous_source: true if either report flagged an AMBIGUOUS source for a primary endpoint value.

Then output STRICTLY in this JSON schema — no preamble, no markdown fences:
...
export async function runPipeline(sourceContext, sourceMeta = {}, adjMeta = {}, isEscalation = false) {
  ...
  // ── Node 1 & 2: sequential dual extraction ───────────────────────────────
  // MUST be sequential — parallel calls trigger per-second concurrency 503s.
  // callGemini handles retries with backoff internally.

  console.log(`[Pipeline] Extractors using: ${model}`);
  console.log(`[Pipeline] Running Extractor A...`);
  const reportA = capOutput(await callGemini(model, {
    systemInstruction: EXTRACTOR_PROMPT_A + escalationWarning,
    contents:          [{ role: 'user', parts: [{ text: sourceContext }] }],
    temperature:       0.05,
  }));
  console.log(`[Pipeline] Running Extractor B...`);
  const reportB = capOutput(await callGemini(model, {
    systemInstruction: EXTRACTOR_PROMPT_B + escalationWarning,
    contents:          [{ role: 'user', parts: [{ text: sourceContext }] }],
    temperature:       0.05,
  }));
  ...
  console.log(`[Pipeline] Adjudicator using: ${model}`);
  const adjText = await callGemini(model, {
    systemInstruction: adjudicatorPrompt,
    contents:          [{ role: 'user', parts: [{ text: adjInput }] }],
