...
export async function runPipeline(sourceContext, sourceMeta = {}, adjMeta = {}, isEscalation = false) {
  ...
  if (process.env.OPENAI_API_KEY) {
    // Different providers — safe to parallelise
    [rawA, rawB] = await Promise.all([
      callGemini(extractModel, EXTRACTOR_PROMPT_A + escalationWarning, sourceContext),
      callOpenAI(EXTRACTOR_PROMPT_B + escalationWarning, sourceContext),
    ]);
  } else {
    // Same provider — sequential to avoid concurrency 503s
    rawA = await callGemini(extractModel, EXTRACTOR_PROMPT_A + escalationWarning, sourceContext);
    rawB = await callGemini(extractModel, EXTRACTOR_PROMPT_B + escalationWarning, sourceContext);
  }
