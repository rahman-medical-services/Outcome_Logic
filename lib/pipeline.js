...
export async function runPipeline(sourceContext, sourceMeta = {}, adjMeta = {}, isEscalation = false) {
  ...
  try {
    ...
    if (_isOverloaded(msg)) {
      ...
      console.warn(`[Pipeline] ${model} overloaded — retrying with ${GEMINI_MODEL_LITE}`);
      try {
  ...
  try {
    ...
    if (_isOverloaded(msg)) {
      // Primary model overloaded — retry once with lite fallback
      console.warn(`[Pipeline] Adjudicator: ${model} overloaded — retrying with ${GEMINI_MODEL_LITE}`);
      try {
