...
5. SECONDARY ENDPOINTS — for each pre-specified secondary endpoint, extract ALL of:
   - Name and exact definition
   - Numerical result (e.g. "34% vs 19% at 5 years", "median PFS 8.2 vs 5.4 months")
   - HR or OR with 95% CI if time-to-event
   - RR or RD with 95% CI if binary outcome
   - Exact p-value
   - Whether statistically significant
   Limit to the 4 most clinically important. Do not write vague summaries — include the numbers.
...
8. ADVERSE EVENTS — CRITICAL — read all sub-rules carefully:
   - For drug/intervention trials: list each Grade ≥3 AE occurring in ≥5% of EITHER arm
   - For each AE, provide percentage for BOTH arms where reported
   - INTERVENTION-ONLY AEs: if an AE is only meaningful or reported for one arm (e.g. chemotherapy
     toxicities in a surgery-vs-chemo trial where surgery has no chemotherapy), set control_pct to null
     and add note "intervention arm only — not applicable to control"
   - Do NOT fabricate 0% for the control arm if the paper does not report that figure for the control.
     Only write 0% if the paper explicitly states zero events in the control arm.
   - For observational/diagnostic studies (e.g. CPET, biomarker, risk-score studies): set has_data to
     false. These studies do not have treatment adverse events. Do not report surgical complication rates
     as AEs unless explicitly framed as such by the authors.
   - Treatment-related mortality rate for both arms (null if not reported, not 0%)
   - Discontinuation due to AE rate for both arms (null if not reported, not 0%)
      ...
      "rows": [
        {
          ...
          "intervention_pct": 0,
          "control_pct": 0,
          "note": "String or null — use 'intervention arm only' if control_pct is null because AE is not applicable to control arm"
        }
      ],
      "discontinuation": {
        "intervention_pct": 0,
...
- Use 'bar' for rates, proportions, and binary outcome data
- chart_priority 1 = primary endpoint (always include if data supports a chart)
- chart_priority 2 = second chart — include for a clinically important secondary endpoint (e.g. a key
  safety signal, DFS when OS is primary, or a major secondary binary outcome). Use 'bar' if it is a
  rate/proportion outcome, 'stepped-line' if it is a survival endpoint.
- Maximum 2 endpoints in the array
- BOTH ARMS ARE MANDATORY: every endpoint in the array MUST have exactly 2 arm objects — one for
  each trial arm. If you include a chart, you must populate data_points for BOTH arms.
  A chart with only one arm will be rejected. If you cannot reconstruct data for both arms, omit
  the chart entirely rather than including a single-arm chart.
- For stepped-line: provide 4-6 data points per arm to form a proper curve; y-values must be
  numeric proportions (0.95 not 95%). Use time points at which survival estimates are reported
  in the text (e.g. "at 12, 24, 36, 48 months").
- For bar: y-values are percentages as integers (e.g. 45 not 0.45). Each arm has a single
  data_point with x = arm label, y = percentage.
...
- If no AE data available, or this is an observational/diagnostic study, set has_data to false and rows to []
- control_pct must be null (not 0) for:
    (a) single-arm trials
    (b) AEs that are mechanistically intervention-specific and not reported in the control arm
        (e.g. chemotherapy toxicities in a surgery-vs-chemo trial — surgery cannot cause neutropenia)
    In these cases set note to "intervention arm only"
- Only set control_pct to 0 if the paper explicitly states zero events in the control arm
- discontinuation and treatment_related_mortality: use null for any value not explicitly reported
...
function postProcess(parsed, sourceMeta) {
  ...
  // ── Normalise AE rows: coerce undefined → null, never fabricate 0 ─────────
  const ae = parsed.clinician_view.adverse_events;
  if (ae.rows) {
    ae.rows = ae.rows.map(row => ({
      ...row,
      intervention_pct: row.intervention_pct ?? null,
      control_pct:      row.control_pct      ?? null,
    }));
    // If ALL control_pct are null, mark clearly for the UI
    const allNullControl = ae.rows.every(r => r.control_pct === null);
    if (allNullControl) ae._control_not_applicable = true;
  }
  if (ae.discontinuation) {
    ae.discontinuation.intervention_pct = ae.discontinuation.intervention_pct ?? null;
    ae.discontinuation.control_pct      = ae.discontinuation.control_pct      ?? null;
  }
  if (ae.treatment_related_mortality) {
    ae.treatment_related_mortality.intervention_pct = ae.treatment_related_mortality.intervention_pct ?? null;
    ae.treatment_related_mortality.control_pct      = ae.treatment_related_mortality.control_pct      ?? null;
  }
