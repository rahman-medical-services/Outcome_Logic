...
export async function runPipeline(sourceContext, sourceMeta = {}, adjMeta = {}, isEscalation = false) {
  ...
  // Uses gpt-4o-mini (OpenAI) for cross-model diversity. A and B use different model
  // families so correlated table misreads produce discrepancies the adjudicator can flag.
  // Falls back to Gemini if OPENAI_API_KEY is not configured.
  console.log(`[Pipeline] Extractor B starting (${process.env.OPENAI_API_KEY ? OPENAI_MODEL_B : extractModel})`);
  const rawB = process.env.OPENAI_API_KEY
    ? await callOpenAI(EXTRACTOR_PROMPT_B + escalationWarning, sourceContext)
    : await callGemini(extractModel, EXTRACTOR_PROMPT_B + escalationWarning, sourceContext);
