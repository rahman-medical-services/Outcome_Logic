...
export async function runPipeline(sourceContext, sourceMeta = {}, adjMeta = {}, isEscalation = false) {
  ...
  // thinkingBudget: 0 disables chain-of-thought on gemini-2.5-flash (enabled by
  // default since early 2026). Thinking adds 10-30s per call — with 3 calls in
  // the pipeline this easily exceeds the 60s Vercel timeout. Extraction tasks
  // don't benefit from thinking; speed and instruction-following matter more.
  ...
  try {
    ...
    if (msg.includes('429') || msg.includes('rate') || msg.includes('quota') ||
      ...
      throw new Error('GEMINI_UNAVAILABLE: The Gemini AI service is experiencing high demand and is temporarily unavailable. Please wait a minute and try again.');
    }
    throw err;
  }
