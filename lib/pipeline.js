// lib/pipeline.js
// Shared 4-node OutcomeLogic analysis pipeline.
// Imported by api/analyze.js and api/library-batch.js — single source of truth.
//
// Node 1 & 2: parallel dual extraction (Gemini Flash, temp 0.1 / 0.2)
// Node 3:     adjudicator (Gemini Flash/Pro, temp 0.0, JSON mode)
// Node 4:     expert context & commentary search (runs in parallel with Node 3)
//
// v4.2.0 — Node 4 commentary integration

import { GoogleGenerativeAI } from '@google/generative-ai';
import { fetchExpertContext }  from './commentary.js';
...
6. SURVIVAL / TIME-TO-EVENT DATA
   ...
   You are STRICTLY FORBIDDEN from calculating or estimating survival percentages using
   "Number at Risk" tables. This produces dangerously wrong values because censored patients
   are mathematically indistinguishable from deaths at coarse time intervals.
   DO NOT do the calculation: survival = (number at risk) / (baseline N). This is wrong.

   Instead, extract ONLY what is EXPLICITLY STATED in the narrative text:

   A) TEXT-STATED SURVIVAL PERCENTAGES — extract only if explicitly written in the paper body:
      - e.g. "2-year overall survival was 74% (95% CI 69–78) in the surveillance arm"
      - e.g. "5-year survival rate 38% vs 24%"
      - Include the timepoint, percentage, and CI if given
      - If a percentage is NOT explicitly written in the text, DO NOT include it — output nothing

   B) NUMBER AT RISK TABLE — extract as structured data (this is the reliable part):
      - List each timepoint and the number at risk per arm
      - Also extract the number censored per arm at each timepoint if shown
      - This is text, not graph reading — extract it accurately

   C) SUMMARY STATISTICS from narrative text:
      - Median survival per arm with 95% CI
      - HR with 95% CI and p-value
      - Any explicitly stated milestone survival rates (e.g. 1-year, 2-year, 5-year OS)
...

{
  ...
  "clinician_view": {
    ...
    "adverse_events": {
      "has_data": true,
      "ae_population_note": "String or null — for organ-sparing trials, explain whose AEs these are e.g. 'Surgical complications in patients who underwent oesophagectomy: surveillance n=83, surgery n=101'",
      "rows": [
        {
...
- Use 'point-estimate' for ALL time-to-event / survival data — never 'stepped-line'
- Use 'bar' for rates, proportions, response data, and non-survival binary outcomes
- chart_priority 1 = primary endpoint (always include if data supports a chart)
- chart_priority 2 = second chart — ONLY include if there is a meaningful pre-specified secondary
  endpoint with sufficient text-stated data, clinically distinct from the primary
- Maximum 2 endpoints in the array

- For 'point-estimate' (survival endpoints):
  * data_points contain ONLY survival percentages explicitly stated in the paper narrative text
  * x = timepoint label as written (e.g. "2 years", "5 years", "24 months")
  * y = survival percentage as integer (e.g. 74 not 0.74)
  * ci_lower and ci_upper = confidence interval bounds as integers (or null if not stated)
  * Include only timepoints that are EXPLICITLY STATED IN THE TEXT — do not read off graphs
  * numbers_at_risk: extract the full risk table if present (timepoints, n per arm, censored per arm)
  * If fewer than 2 text-stated timepoints exist per arm, omit this endpoint from the array
  ⚠️ FORBIDDEN: Do NOT calculate survival as (number at risk / baseline N). This is wrong.

- For 'bar':
  * y-values are percentages as integers (e.g. 45 not 0.45)
  * ci_lower and ci_upper are null unless CIs are explicitly stated
  * numbers_at_risk is null for bar charts

- If insufficient data for any chart, output an empty endpoints array
...
- For organ-sparing, watch-and-wait, or active surveillance trials: the AE table may only
  apply to the surgical subgroup (those who eventually underwent the procedure), NOT the
  full trial population. In this case, set ae_population_note to explain the denominator
  clearly (e.g. "Applies to patients undergoing oesophagectomy: surveillance n=83, surgery n=101").
  Do NOT present surgical complication rates as if they apply to all patients in the arm.
...
function postProcess(parsed, sourceMeta, expertContext) {
  ...
  // ── Merge expert_context from Node 4 ─────────────────────────────────────
  // Only attach if Node 4 ran and returned something useful.
  // pmid_unresolved and error are silently omitted from the output.
  console.log('[postProcess] expertContext status:', expertContext?.status);
  if (expertContext && expertContext.status === 'found') {
    parsed.clinician_view.expert_context = expertContext;
  } else if (expertContext && expertContext.status === 'not_found') {
    parsed.clinician_view.expert_context = expertContext;
  }
  // status: pmid_unresolved | error | undefined → field absent from output
...
export async function runPipeline(sourceContext, sourceMeta = {}, adjMeta = {}, isEscalation = false) {
  ...
  // The PDF text is deterministic — scan it directly rather than relying on
  // extractor output (which varies with temperature). Inject into sourceMeta
  // so Node 4 can use it reliably without falling back to name-based search.
  if (sourceMeta.sourceType === 'full-text-pdf' && !sourceMeta.pmid && !sourceMeta.doi) {
    // Scan more of the text — DOI and PMID are often in headers, footers, or
    // reference sections. Check first 4000 chars AND last 2000 chars.
    ...
    // DOI — multiple patterns including bare 10.XXXX and Lancet-style with parens
    if (!sourceMeta.pmid) {
  ...
  // ── Nodes 1 & 2: parallel dual extraction ────────────────────────────────
  ...
  // ── Nodes 3 & 4: fire in parallel ────────────────────────────────────────
  // Node 3: adjudicator (existing behaviour, unchanged)
  // Node 4: commentary search — always catches, never propagates errors
  ...
  const [adjResponse, expertContext] = await Promise.all([

    adj.generateContent(adjInput),

    fetchExpertContext(sourceMeta, reportA).catch(err => {
      // Belt-and-suspenders: fetchExpertContext already catches internally,
      // but we catch here too so a bug in commentary.js never kills the pipeline.
      console.warn('[Pipeline] Node 4 outer catch:', err.message);
      return { status: 'error', reason: err.message };
    }),

  ]);

  const parsed = JSON.parse(adjResponse.response.text());

  return postProcess(parsed, sourceMeta, expertContext);
}

// ─────────────────────────────────────────────
// SINGLE-NODE BASELINE: runPipelineV1
// Used for Phase 1/2 validation study only.
// One extractor, no adjudicator, no schema enforcement, no Node 4.
// Raw output stored as-is — the messiness is the data point.
// ─────────────────────────────────────────────
export async function runPipelineV1(sourceContext, sourceMeta = {}) {
  ...
  const result = await extractor.generateContent({
    contents:         [{ role: 'user', parts: [{ text: sourceContext }] }],
    generationConfig: { temperature: 0.1 },
  });
  ...
  // Attempt JSON parse but do not enforce or clean up.
  // Store whatever comes back — schema failures are valid error signals.
  let parsed;
  try {
    const clean = raw.replace(/```json|```/gi, '').trim();
    parsed = JSON.parse(clean);
  } catch {
    // V1 returned unparseable output — record this explicitly.
    parsed = {
      _v1_parse_error: true,
      _v1_raw_output:  raw,
    };
  }

  // No postProcess — raw V1 output is intentionally unprocessed.
  // Cleaning it would mask exactly the errors we are measuring.
  parsed._provenance = {
    source:    sourceMeta.sourceType || 'unknown',
    model:     GEMINI_MODEL,
    version:   'v1',
