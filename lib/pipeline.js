...
// Raw fetch wrapper — bypasses SDK entirely (same pattern as search.js)
...
const GEMINI_MODEL_LITE    = 'gemini-2.5-pro';           // fallback when 2.5-flash is overloaded
...
export async function runPipeline(sourceContext, sourceMeta = {}, adjMeta = {}, isEscalation = false) {
  ...
  // gemini-2.5-flash uses thinking by default (~16 tokens overhead).
  // thinkingBudget:0 returns 503 — do NOT disable thinking.
  // On 503 overload, waits 3s then retries with Pro fallback.
  ...
  async function _runExtractors(extractModel) {
    console.log(`[Pipeline] Running Extractor A...`);
    const textA = await callGemini(extractModel, {
    ...
    const textB = await callGemini(extractModel, {
      systemInstruction: EXTRACTOR_PROMPT_B + escalationWarning,
      contents:          [{ role: 'user', parts: [{ text: sourceContext }] }],
      temperature:       0.05,
    });
    return [capOutput(textA), capOutput(textB)];
  }

  let reportA, reportB;
  try {
    console.log(`[Pipeline] Extractors using: ${model}`);
    [reportA, reportB] = await _runExtractors(model);
  } catch (err) {
    console.error('[Pipeline] Extractor error:', err.message);
    const msg = err.message?.toLowerCase() || '';
    if (_isOverloaded(msg)) {
      // Primary model overloaded — wait 3s then retry with Pro fallback
      console.warn(`[Pipeline] ${model} overloaded — waiting 3s then retrying with ${GEMINI_MODEL_LITE}`);
      await _sleep(3000);
      try {
  ...
  async function _runAdjudicator(adjModel) {
    const text = await callGemini(adjModel, {
  ...
  try {
    ...
    parsed = await _runAdjudicator(model);
  } catch (err) {
    console.error('[Pipeline] Adjudicator error:', err.message);
    const msg = err.message?.toLowerCase() || '';
    if (_isOverloaded(msg)) {
      // Primary model overloaded — retry once with lite fallback
      console.warn(`[Pipeline] Adjudicator: ${model} overloaded — waiting 3s then retrying with ${GEMINI_MODEL_LITE}`);
      await _sleep(3000);
      try {
        parsed = await _runAdjudicator(GEMINI_MODEL_LITE);
        console.log(`[Pipeline] Adjudicator lite fallback succeeded`);
      } catch (fallbackErr) {
        console.error('[Pipeline] Adjudicator lite fallback failed:', fallbackErr.message);
        const fbMsg = fallbackErr.message?.toLowerCase() || '';
        // SyntaxError means the lite model returned truncated/malformed JSON under stress —
        // treat it the same as a capacity error rather than letting it surface as a 500.
