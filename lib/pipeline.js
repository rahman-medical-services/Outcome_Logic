...
8. ADVERSE EVENTS — CRITICAL — read all sub-rules carefully:
   ...
   - INTERVENTION-ONLY AEs: if an AE is mechanistically specific to one arm (e.g. chemotherapy
     toxicities in a surgery-vs-chemo trial — surgery cannot cause neutropenia), set control_pct
     to null and add note "intervention arm only"
...
- chart_priority 2 = second chart — include for a clinically important secondary endpoint
  (e.g. DFS when OS is primary, a key safety rate, a major binary secondary outcome)
- Maximum 2 endpoints in the array
- BOTH ARMS MANDATORY: every endpoint MUST have exactly 2 arm objects — one per trial arm.
  If you can only reconstruct data for one arm, omit the chart entirely rather than producing
  a single-arm chart. A chart with one arm will render incorrectly.
- For stepped-line: provide 4-6 data points per arm; y-values are proportions 0.0–1.0 (not %)
  Use time points at which survival estimates are explicitly stated in the text.
- For bar: each arm has a single data_point: { x: "arm label", y: integer percentage }
...
- If no AE data, or this is an observational/diagnostic study, set has_data to false and rows to []
- Set control_pct to null (not 0) when:
    (a) single-arm trial
    (b) AE is mechanistically intervention-specific and not reported for the control arm
    In these cases add note "intervention arm only"
