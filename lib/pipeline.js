...
export async function runPipeline(sourceContext, sourceMeta = {}, adjMeta = {}, isEscalation = false) {
  ...
  // Model selection:
  // - Normal mode: Flash for extractors (speed), Pro for adjudicator (accuracy)
  // - Escalation mode: Pro for all nodes (maximum accuracy on retry)
  const extractorModel   = isEscalation ? GEMINI_MODEL_PRO : GEMINI_MODEL;
  const adjudicatorModel = GEMINI_MODEL_PRO; // Always Pro for final synthesis
  
  console.log(`[Pipeline] Mode: ${isEscalation ? 'ESCALATION' : 'normal'}`);
  console.log(`[Pipeline] Extractors (Node 1&2): ${extractorModel}`);
  console.log(`[Pipeline] Adjudicator (Node 3): ${adjudicatorModel}`);
  ...
  const eA = genAI.getGenerativeModel({ model: extractorModel, systemInstruction: EXTRACTOR_PROMPT + escalationWarning });
  const eB = genAI.getGenerativeModel({ model: extractorModel, systemInstruction: EXTRACTOR_PROMPT + escalationWarning });
  ...
  // ── Node 3: adjudication (always Pro for highest accuracy) ────────────────
