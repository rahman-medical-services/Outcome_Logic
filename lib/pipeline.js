...
// CONSTANTS
// ─────────────────────────────────────────────
const GEMINI_MODEL         = 'gemini-2.5-flash-lite'; // primary — stable, avoids TPM exhaustion
const GEMINI_MODEL_FLASH   = 'gemini-2.5-flash';       // escalation path only
...
// thinkingBudget: 512 on all calls — prevents TPM exhaustion, satisfies flash-lite minimum.
...
export async function runPipeline(sourceContext, sourceMeta = {}, adjMeta = {}, isEscalation = false) {
  ...
  // Escalation uses flash instead of flash-lite for higher reasoning capacity.
  const extractModel = isEscalation ? GEMINI_MODEL_FLASH : GEMINI_MODEL;
  ...
  // ── Node 1: Extractor A — adjusted/ITT/methods-specified priority ─────────
  // Sequential — NOT parallel. Parallel calls trigger concurrency 503s on this API key.
  console.log(`[Pipeline] Extractor A starting (${extractModel})`);
  const rawA = await callGemini(
    extractModel,
    EXTRACTOR_PROMPT_A + escalationWarning,
    sourceContext,
  );
  const { text: reportA, truncated: truncatedA } = capOutput(rawA);
  console.log(`[Pipeline] Extractor A complete${truncatedA ? ' [TRUNCATED]' : ''}`);

  // ── Node 2: Extractor B — first-reported/results-section priority ─────────
  console.log(`[Pipeline] Extractor B starting (${extractModel})`);
  const rawB = await callGemini(
    extractModel,
    EXTRACTOR_PROMPT_B + escalationWarning,
    sourceContext,
  );
  ...
  const truncationNote = (truncatedA || truncatedB)
    ? `\nTRUNCATION NOTICE:${truncatedA ? ' Extractor A output was truncated at character limit.' : ''}${truncatedB ? ' Extractor B output was truncated at character limit.' : ''} Treat any field absent from a truncated report as UNKNOWN — do not infer from absence. Do not flag omissions in truncated fields as extraction errors.`
    : '';
