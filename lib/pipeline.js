...
export async function runPipeline(sourceContext, sourceMeta = {}, adjMeta = {}, isEscalation = false) {
  ...
  // thinkingBudget: 0 disables chain-of-thought (enabled by default on 2.5-flash
  // since early 2026) — adds 10-30s per call and exceeds Vercel 60s timeout.
  // On 503 overload, retries once then falls back to gemini-2.5-flash-lite.
  // gemini-2.5-pro requires thinking mode — thinkingBudget:0 is only valid for Flash
  function _thinkingConfig(m) {
    return m.includes('pro') ? {} : { thinkingConfig: { thinkingBudget: 0 } };
  }
  ...
  async function _runExtractors(extractModel) {
    ...
    return [capOutput(rA.text), capOutput(rB.text)];
  }
