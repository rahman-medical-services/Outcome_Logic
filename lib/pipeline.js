...
// ─────────────────────────────────────────────
// SHARED EXTRACTOR INSTRUCTIONS
// Core extraction schema — identical for both A and B.
// Priority headers and source citation rules are prepended per-extractor.
// ─────────────────────────────────────────────
const EXTRACTOR_CORE = `You are an elite Clinical Data Extraction Agent analyzing a FULL-TEXT clinical trial.
...
// ─────────────────────────────────────────────
// EXTRACTOR A — Priority: adjusted estimates / ITT / methods-specified primary analysis
// Rationale: will disagree with B on ambiguous papers (adjusted vs unadjusted HR,
// ITT vs per-protocol). Disagreement is the signal that drives adjudicator scrutiny.
// ─────────────────────────────────────────────
const EXTRACTOR_PROMPT_A = `EXTRACTION PRIORITY — READ THIS FIRST:
Prefer adjusted estimates over unadjusted. If both an adjusted and unadjusted effect size are reported (e.g. crude HR and adjusted HR), extract the adjusted value as the primary result and note the unadjusted as secondary.
If ITT and per-protocol results both appear, extract ITT.
Select the primary analysis result as pre-specified in the methods section — not the first value that appears in the abstract.
If multiple effect sizes exist and you are uncertain which is the pre-specified primary, extract the one most likely intended as the primary analysis AND flag: "(SELECTION_UNCERTAIN — methods not explicit)".

` + EXTRACTOR_CORE;

// ─────────────────────────────────────────────
// EXTRACTOR B — Priority: first-reported / results-section / as-presented
// Rationale: will extract the first-presented value, creating productive disagreement
// with A when papers report adjusted and unadjusted estimates in different orders.
// ─────────────────────────────────────────────
const EXTRACTOR_PROMPT_B = `EXTRACTION PRIORITY — READ THIS FIRST:
Prefer the primary reported outcome exactly as labelled in the results section, regardless of statistical adjustment.
If multiple effect sizes appear, extract the first one reported in the abstract results section.
Do not infer primacy from the methods section — extract what is presented first in the results.
If only one effect size appears in the abstract but multiple appear in the results section body, extract the abstract value and flag: "(ABSTRACT_ONLY — full results may differ)".
...
export async function runPipeline(sourceContext, sourceMeta = {}, adjMeta = {}, isEscalation = false) {
  ...
  const adjModel = GEMINI_MODEL_PRO;
