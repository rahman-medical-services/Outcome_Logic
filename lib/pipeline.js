...
- Use 'stepped-line' for ANY time-to-event outcome: overall survival, disease-free survival,
  relapse-free survival, progression-free survival, or any KM curve data — EVEN if you only
  have a single time-point value (e.g. "5-year OS 38% vs 24%") — reconstruct as a curve
- Use 'bar' ONLY for outcomes that are NOT time-to-event: single binary event rates
  (e.g. "DVT/PE within 90 days: 1.7% vs 1.4%"), response rates, or proportions with no
  time component. If the outcome name contains "survival", "free", "recurrence", or
  "mortality" it is almost always 'stepped-line'.
- chart_priority 1 = primary endpoint
- chart_priority 2 = a clinically distinct secondary endpoint with different data.
  Do NOT output two charts with the same label or same underlying data.
- Maximum 2 endpoints. BOTH ARMS MANDATORY in every endpoint.
  If you cannot reconstruct both arms, omit the chart entirely.
- For stepped-line: 4-6 data_points per arm; y-values are proportions 0.0–1.0
...
- If no AE data, or observational/diagnostic study, set has_data false and rows []
- Set control_pct to null (not 0) when AE is mechanistically intervention-specific.
  Add note "intervention arm only".
