...
8. ADVERSE EVENTS — Grade 3 or higher only:
   CLASSIFICATION RULE — MANDATORY: The AE table must contain ONLY procedure-related or drug-related complications.
   Do NOT list primary endpoints, secondary endpoints, or pre-specified trial outcome events in this table — even if
   they appear in a safety section or safety table in the paper. Events such as death, stroke, MI, atrial fibrillation,
   or any outcome that is defined as a trial endpoint belong in the outcomes section, not here. If a safety table in
   the paper mixes AEs with endpoints, extract only the true complications (e.g. wound infection, bleeding requiring
   transfusion, acute kidney injury, sepsis) and exclude the endpoint events.
   - List each Grade ≥3 AE (true complication) occurring in ≥2% of either arm
...
The adverse_events table must contain ONLY procedure-related or drug-related complications.
Do NOT list primary endpoints, secondary endpoints, or pre-specified trial outcomes in this table —
even if they appear in a safety table in the paper. Events such as death, stroke, MI, or AF that are
defined as trial endpoints belong in the outcomes section, not the AE table. If these appear in the
extractor AE lists, exclude them from adverse_events.rows.
