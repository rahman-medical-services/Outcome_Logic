...
// CONSTANTS
// ─────────────────────────────────────────────
// NOTE: thinkingConfig/thinkingBudget is not supported in @google/generative-ai SDK.
// TODO: migrate to @google/genai SDK for proper thinking control on gemini-2.5-flash.
const GEMINI_MODEL         = 'gemini-2.5-flash';
const GEMINI_MODEL_LITE    = 'gemini-1.5-flash-latest'; // fallback when primary is overloaded
const GEMINI_MODEL_PRO     = 'gemini-2.5-pro';         // escalation path only
...
export async function runPipeline(sourceContext, sourceMeta = {}, adjMeta = {}, isEscalation = false) {
  ...
  const adjModel = extractModel;  // Flash normally, Pro on escalation — matches extractor behaviour
    ...
    if (_isOverloaded(msg)) {
      ...
      try {
        ...
        if (_isCapacityError(fbMsg) || fallbackErr instanceof SyntaxError) {
          throw new Error('GEMINI_UNAVAILABLE: Gemini API quota limit reached — your project may have exhausted its RPM allowance. Check Google Cloud Console → Gemini API → Quotas, or wait a few minutes and retry.');
        }
        throw fallbackErr;
      }
