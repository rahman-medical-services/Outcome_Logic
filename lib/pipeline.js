...
// CONSTANTS
// ─────────────────────────────────────────────
const GEMINI_MODEL         = 'gemini-2.5-flash-lite';  // primary — higher throughput quota than flash
...
3. BASELINE CHARACTERISTICS
   ...
   - Follow-up duration: extract as structured value (e.g. "median follow-up 3.2 years" → value=3.2, unit=years, type=median)

4. PRIMARY ENDPOINT — extract all of the following:
   - Outcome type: classify as binary (event occurred/not), continuous (measured score/value), or time_to_event (HR-based survival analysis)
   ...
   - Outcome assessment timepoint as structured value (e.g. "30-day mortality" → value=30, unit=days; "12-month function score" → value=12, unit=months)
   - For binary outcomes: extract BOTH the event count as integer AND the percentage for each arm (e.g. "47/312 (15.1%)" → events=47, n=312, rate=15.1%)
   - For continuous outcomes: extract mean (or median), SD (or IQR), and N per arm
   ...
   - NON-INFERIORITY TRIALS: If the primary analysis uses a non-inferiority design, you MUST additionally extract:
     ...
     NI CI DIRECTION — apply this rule exactly:
     For HRs and RRs (where <1 favours intervention): CI upper bound < NI margin → CI excludes margin → NI demonstrated.
     CI upper bound > NI margin → CI includes margin → NI not demonstrated.
     For risk differences: CI upper bound < NI margin → CI excludes margin → NI demonstrated.
     Do NOT invert this. "CI excludes the NI margin" means the entire CI is on the beneficial side of the margin.
   ...
   Format immediately after the main effect size extraction:
     candidate_values:
       1. value=X.XX | effect_measure=[HR/OR/RR/MD/RD/difference] | outcome_type=[binary/continuous/time_to_event] | p=[p-value or 'not reported'] | timepoint=[value unit, e.g. '30 days' or '12 months'] | label="[e.g. adjusted HR, Cox model, Table 2]" | population=[ITT/PP/mITT] | arm_a=[GROUP_NAME: X.XX UNITS, events=N/total] | arm_b=[GROUP_NAME: X.XX UNITS, events=N/total] | [SRC: ...]
       2. value=X.XX | effect_measure=[HR/OR/RR/MD/RD/difference] | outcome_type=[binary/continuous/time_to_event] | p=[p-value or 'not reported'] | timepoint=[value unit] | label="[e.g. unadjusted HR, abstract Results]" | population=[ITT/PP/mITT] | arm_a=[GROUP_NAME: X.XX UNITS, events=N/total] | arm_b=[GROUP_NAME: X.XX UNITS, events=N/total] | [SRC: ...]

   ARM VALUE RULE — CRITICAL — read carefully before extracting arm_a and arm_b:
   arm_a and arm_b are the OBSERVED VALUE IN EACH ARM — NOT the between-group effect size.
   - Binary outcome: arm_a = event rate % in intervention arm (e.g. "PCI: 15.4%"), arm_b = event rate % in control arm
   - Continuous outcome: arm_a = mean or median in intervention arm (e.g. "PCI: 28.4 seconds"), arm_b = mean or median in control arm
   - Change-from-baseline: arm_a = mean change in intervention arm (e.g. "+28.4s"), arm_b = mean change in control arm (e.g. "+11.8s")
   - Time-to-event (HR outcome): arm_a = median survival or event-free survival % at stated timepoint in intervention arm; arm_b = same in control arm
   The between-group effect size (HR, OR, MD, risk difference) belongs in the "value" field — NEVER in arm_a or arm_b.
   If the paper does not report per-arm observed values, write arm_a=null | arm_b=null — do NOT substitute the effect size.
   ...
   - Have I correctly separated arm_a/arm_b (per-arm observed values) from value (between-group effect size)?
...
7. SUBGROUP ANALYSES — TIERED EXTRACTION (Option C):
   If no subgroup analyses are reported in the paper, state explicitly "No subgroup analyses reported."
   COMPLETENESS RULE: Scan ALL figures (especially forest plots) — subgroups may appear in forest plot figures
   only, not in numbered tables. Extract from both.

   TIERED EXTRACTION RULE — apply in order:
   STEP 1: Check all reported interaction p-values across all subgroups.
   STEP 2:
   - If ANY interaction p < 0.10 (significant or borderline): extract ALL pre-specified subgroups that meet
     this threshold. Include both significant (p<0.05) and borderline (0.04–0.10) interactions.
   - If ALL interaction p-values are null or p ≥ 0.10: you MUST STILL extract exactly 4 pre-specified
     subgroups — null interactions do NOT mean zero subgroups. Select in this clinical priority order:
       (1) Age (e.g. age <65 vs ≥65, youngest vs oldest quartile)
       (2) Sex (male vs female)
       (3) Disease severity measure (domain-specific: STS-PROM, LVEF, NYHA class, tumour stage, fracture type)
       (4) One key domain comorbidity (renal function, prior therapy, prior revascularisation)
     If fewer than 4 of these are reported in the paper, extract however many exist.
   STEP 3: POST-HOC subgroups — extract only if interaction p < 0.05. Do not extract exploratory post-hoc
   subgroups with p ≥ 0.05.
   HARD CAP: Maximum 8 subgroups total in output regardless of how many are reported.

   GROUPING RULE — MANDATORY: All arms belonging to the same variable MUST be grouped under a single item.
   Example: Age <75 and Age ≥75 are TWO ARMS of ONE item (variable = "Age"). Do NOT create a separate item
   for each stratum — one item per variable, multiple arms within it. An item with only one arm is invalid.

   For each extracted subgroup:
   - Flag as pre-specified or post-hoc
   - State the subgroup variable (e.g. "Age ≥75 years", "STS-PROM ≥4%", "LVEF <35%")
   - Interaction p-value (or "not reported" if absent)
   - If interaction p is between 0.04 and 0.06, flag as BORDERLINE
   - For each subgroup arm: effect estimate (HR, OR, MD, or mean difference as reported), 95% CI, and absolute event counts
   - Whether all individual subgroup CIs cross 1 (no individual arm statistically significant)
   - If observed direction contradicts the pre-specified hypothesis, note this explicitly
...
8. ADVERSE EVENTS — Grade 3 or higher only:
   CLASSIFICATION RULE — MANDATORY: The AE table must contain ONLY unscheduled procedure-related or
   drug-related complications that are NOT pre-specified trial outcomes.
   CROSS-REFERENCE (apply before finalising this section):
   (a) List every component of the primary composite outcome from Section 4.
   (b) List every secondary endpoint you extracted in Section 5.
   (c) Remove from your AE candidate list ANY event that matches (a) or (b) — regardless of whether it
       also appears in the paper's safety table. An event that is a pre-specified trial endpoint belongs
       in the outcomes section only, never in the AE table, even if it is also a procedure complication.
   Examples of events that are almost always trial endpoints and must be EXCLUDED: death, stroke, MI,
   repeat revascularisation, atrial fibrillation, new LBBB, new pacemaker implantation, valve
   dysfunction (if pre-specified), renal failure (if pre-specified). Include these ONLY if they are
   explicitly NOT pre-specified outcomes in this specific trial.
   Examples of events that are typically true unscheduled complications (INCLUDE if ≥2%): wound
   infection, vascular access-site complication (if not a pre-specified endpoint), bleeding requiring
   transfusion (if not pre-specified), acute kidney injury (if not pre-specified), sepsis, transfusion.
   - List each qualifying AE (true unscheduled complication, not a trial endpoint) in ≥2% of either arm
...
9. CRITICAL APPRAISAL
   ...
   FALLBACK: if detail is absent for a domain, apply the defaults above rather than Moderate for overall.
...
10. CHARTS — explicit arm values for chart rendering
   Always use recommended_chart_type: "bar".
   Populate arms[].data_points[].y with the per-arm OBSERVED EVENT RATE as a raw percentage number:
   - Binary outcome: y = event rate % (e.g. 5.4 for 5.4%, NOT 0.054)
   - Continuous outcome: y = mean or median value in that arm
   - Time-to-event: y = survival % at stated timepoint
   y-values come from arm_a_value and arm_b_value in the primary endpoint.
   After completing Section 4, explicitly state the chart values:
     Chart y-values: [Arm A name] = [arm_a_value]%,  [Arm B name] = [arm_b_value]%
   These must match arm_a_value and arm_b_value from candidate_values — no substitution.
   For multi-timepoint survival: one data point per arm per explicitly reported timepoint. Do not interpolate.
   Maximum 2 endpoints in the array.

11. SOURCE CITATIONS — MANDATORY FOR ALL EXTRACTED VALUES
...
// Runs on GEMINI_MODEL (flash-lite) — same as extractors. Pro not used in current pipeline.
...
NON-INFERIORITY TRIALS — MANDATORY RULES (applies when ni_trial: true):
1. The primary_outcome field in clinician_view.pico MUST lead with: the per-arm event rates, the absolute
   difference, whether the 95% CI excludes the NI margin, and the NI test result. The relative effect measure
   (HR/OR/RR) is supporting context — do NOT lead with it or present it as the primary result.
2. CI vs NI margin interpretation — apply this rule exactly:
   - For HRs and RRs (where <1 favours intervention): NI is DEMONSTRATED when the upper bound of the 95% CI
     is LESS THAN the NI margin. Example: CI upper 0.79, NI margin 1.14 → 0.79 < 1.14 → CI excludes margin → NI demonstrated.
   - For risk differences or absolute measures: NI is demonstrated when the upper bound of the CI is less than the NI margin.
   - "CI excludes the NI margin" means the entire CI is on the NI side — upper bound does not cross the margin.
   - Do NOT invert this: CI upper < NI margin = NI success. CI upper > NI margin = NI failure.
3. Do NOT claim or imply superiority unless the trial pre-specified a superiority test AND it was met.
   NI success alone is not superiority. The phrase "noninferior and superior" is only valid if a formal
   pre-specified superiority test was performed and passed. If in doubt, state NI only.
4. This constraint applies equally to context.what_this_adds — do not use "demonstrating superiority"
   or equivalent phrasing unless a formal superiority test was pre-specified and met.

SUBGROUP COMPILATION — MANDATORY:
Both extractor reports may contain subgroup analyses. You MUST compile ALL subgroups from BOTH reports
into the clinician_view.subgroups.items array. Do not omit subgroups present in either report.
- If both extractors reported the same subgroup, use the more complete version.
- If only one extractor reported a subgroup, include it and note which extractor sourced it.
- If neither extractor reported subgroups, set has_significant_interactions: false and items: [].
- Missing subgroups is a completeness failure — treat subgroup compilation with the same rigour as
  primary endpoint extraction.
- GROUPING RULE: Each item must have ≥2 arms. All strata of the same variable (e.g. Age <75 and Age ≥75)
  MUST be grouped under ONE item. If an extractor created one item per stratum, merge them before output.
  An item with only one arm is invalid — merge it or discard it.

ADVERSE EVENTS — CLASSIFICATION RULE:
The adverse_events table must contain ONLY unscheduled procedure-related or drug-related complications
that are NOT pre-specified trial outcomes. Apply this cross-reference before finalising adverse_events:
(a) Identify every component of the primary composite outcome — if the primary is a SINGLE event (e.g.
    "death from any cause"), that single event is also excluded.
(b) Identify every secondary endpoint extracted by the two extractors.
(c) Remove from adverse_events.rows ANY event matching (a) or (b), even if both extractors placed it
    in their AE lists, and even if the paper's safety table includes it. Pre-specified endpoints belong
    in the outcomes section only. Common events to EXCLUDE (if pre-specified in this trial): death,
    stroke, MI, repeat revascularisation, AF, new LBBB, new pacemaker implantation, major bleeding
    (if a secondary endpoint), renal failure (if a secondary endpoint).
IMPORTANT: If the primary outcome is a COMPOSITE (e.g. death + stroke + MI), ALL individual components
    of that composite are also excluded — not just the composite as a whole.
If applying this rule leaves adverse_events.rows empty, set has_data: false — this is correct for
trials where all reported safety events are pre-specified endpoints.

PATIENT VIEW — REQUIRED FIELDS:
patient_view.lay_summary and patient_view.shared_decision_making_takeaway MUST ALWAYS be populated.
These fields are NEVER optional — every paper has patients who could read it. If either extractor
omitted them, write them now from the available data. A blank patient_view is a completeness failure.
...
"the results suggest", "the data indicate". This applies equally to context.already_known and
context.what_this_adds — these fields must reflect what the trial design actually tested.

CRITICAL APPRAISAL — RECONCILIATION RULES:
When reconciling risk_of_bias across extractor reports:
- NEVER output "Unclear". If either extractor assigned Unclear, override it by applying the Cochrane domain
  defaults: assess randomisation, allocation concealment, blinding, adjudication, and attrition from the
  extractor reports, then assign Low / Moderate / High.
- NEVER output "Very Low" for grade_certainty. If either extractor assigned Very Low for an RCT, override
  to Low (if ≥2 serious limitations) or Moderate (≤1 serious limitation).
- If the extractors disagree on RoB (e.g. one says Low, one says Moderate), favour the better-supported
  assessment — justify from the domain evidence stated in the reports.
- coi_funding: populate verbatim from whichever extractor named the sponsor. If neither named it but the
  text mentions a funding acknowledgement, extract it now. null only if genuinely not reported.
...

{
  ...
  "primary_endpoint_candidates": [
    {
      ...
      "arm_a_value": "OBSERVED value in intervention arm — event rate %, mean, median, or mean change from baseline. NOT the between-group effect size. null if not reported.",
      "arm_a_sd": null,
      "events_arm_a": "Integer — number of events in intervention arm (e.g. 47). For binary outcomes only. null for continuous/TTE.",
      "n_arm_a": "Integer — number of participants analysed in intervention arm (denominator for arm_a_value).",
      "n_randomised_arm_a": "Integer — number of participants randomised to intervention arm. null if not reported separately from analysed N.",
      "arm_b_name": "String — control group label e.g. 'OMT', 'Conservative'",
      "arm_b_value": "OBSERVED value in control arm — same metric as arm_a_value. NOT the between-group effect size. null if not reported.",
      "arm_b_sd": null,
      "events_arm_b": "Integer — number of events in control arm. For binary outcomes only. null for continuous/TTE.",
      "n_arm_b": "Integer — number of participants analysed in control arm.",
      "n_randomised_arm_b": "Integer — number of participants randomised to control arm. null if not reported separately from analysed N.",
      "total_events": "Integer — total events across both arms combined. Required for time-to-event weighting. null if not applicable.",
      "outcome_type": "binary | continuous | time_to_event — classification for meta-analysis pooling method selection",
      "outcome_timepoint": { "value": null, "unit": "months | years | days | weeks — the assessment timepoint for this specific result" },
      "value_type": "rate_pct | mean_score | median | count | time_to_event — what arm_a_value and arm_b_value represent",
      "value_unit": "String or null — e.g. '% events', 'SF-36 points', 'mmHg', 'seconds', 'months'",
      "effect_measure": "HR | OR | RR | RD | MD | SMD | difference | null — what the 'value' field represents (the between-group effect size)",
      "p_value": "String or null — exact p-value as reported e.g. '0.001' or '<0.05' or 'not reported'",
      "ci_lower": 0.0,
      "ci_upper": 0.0,
          ...
          "arms": [
            {
              ...
              "hr": "String — the point estimate for this subgroup (ALWAYS populate: use the HR, OR, RR, MD, or mean difference value as reported — e.g. '0.62' or '5.0'. Do NOT leave null for non-ratio outcomes; populate with whatever numeric effect measure is reported for this subgroup.)",
      ...
      "endpoints": [
        {
          ...
          "axes": { "x_label": "String", "y_label": "Event Rate (%)" },
          "arms": [
            {
              "group_name": "String — arm label e.g. 'TAVI' or 'SAVR'",
              "data_points": [
                {
                  "x": "String — the timepoint label e.g. '1 year' or '30 days'",
                  "y": "Number — the per-arm OBSERVED EVENT RATE as a raw percentage number (e.g. 5.4 for 5.4%, NOT 0.054). Use arm_a_value for the intervention arm and arm_b_value for the control arm from primary_endpoint_candidates. This field MUST be populated — null or 0 will result in a blank chart."
                }
              ]
            }
          ],
  ...
  "library_meta": {
    "domain": "Surgery | Orthopaedics | Medicine | Critical Care | Anaesthesia — IMPORTANT: Orthopaedics = any musculoskeletal, joint, bone, fracture, or spine trial regardless of whether the treatment is surgical or non-surgical. Surgery = general/visceral/vascular/breast/endocrine/colorectal surgery only. A trial comparing hip replacement vs physiotherapy is Orthopaedics, not Surgery.",
    "specialty": "Must exactly match one of the valid specialties for the chosen domain listed in VALID TAXONOMY below",
...
- If suspicious_agreement: true AND only one extractor provided a source citation for the agreed value, ALSO set selection_uncertain: true (single-source agreement is unreliable)
- Set zero_event_arm: true if either arm of the primary outcome has zero events (breaks OR/RR calculation — flag for downstream meta-analysis handling)
- Set multi_arm_trial: true if the trial has more than two randomised arms (e.g. three active doses, or two interventions vs placebo) — flags need for control group selection decision in meta-analysis
...
- Only include Grade ≥3 AEs occurring in ≥2% of either arm
...
- GROUPING RULE: All arms of the same variable MUST be under one item. Merge any single-arm items that
  belong to the same variable. An item with one arm is invalid output.
- hr field: ALWAYS populate with whatever numeric effect measure is reported for this subgroup (HR, OR,
  RR, MD, mean difference). Do NOT leave null for non-ratio outcomes — use the reported numeric value.
...
// ─────────────────────────────────────────────
export function postProcess(parsed, sourceMeta) {
  ...
  if (ca) {
    ...
    const validRoB   = ['Low', 'Moderate', 'High'];
    if (!validGrade.includes(ca.grade_certainty)) ca.grade_certainty = 'Moderate';
    if (!validRoB.includes(ca.risk_of_bias))      ca.risk_of_bias    = 'Moderate';
  }
  ...
  // ── Ensure patient_view fields exist ─────────────────────────────────────
  if (!parsed.patient_view) parsed.patient_view = {};
  if (!parsed.patient_view.lay_summary || parsed.patient_view.lay_summary.length < 20) {
    const po = parsed.clinician_view?.pico?.primary_outcome || '';
    const trialId = parsed.reportMeta?.trial_identification || 'This trial';
    parsed.patient_view.lay_summary = `${trialId} examined whether one treatment was as good as or better than another. ${po.slice(0, 200)}`.trim();
    console.warn('[postProcess] lay_summary was missing — fallback generated');
  }
  if (!parsed.patient_view.shared_decision_making_takeaway || parsed.patient_view.shared_decision_making_takeaway.length < 20) {
    parsed.patient_view.shared_decision_making_takeaway = 'These results should be discussed with your clinical team in the context of your individual circumstances, preferences, and other available treatments.';
    console.warn('[postProcess] sdm_takeaway was missing — fallback generated');
  }
  ...
  if (!currentEndpoints?.length) {
    ...
    if (selected && selected.arm_a_value != null && selected.arm_b_value != null) {
      // Sanity check: if both arms show same value, ARM VALUE RULE may have been violated
      if (selected.arm_a_value === selected.arm_b_value && selected.arm_a_value !== 0) {
        console.warn('[postProcess] Synthetic endpoint: arm_a_value === arm_b_value — possible ARM VALUE RULE violation (effect size used instead of per-arm rate?)');
      }
      const armAName = selected.arm_a_name || pico.intervention || 'Intervention';
      const armBName = selected.arm_b_name || pico.control      || 'Control';
      const yLabel   = selected.value_unit || (selected.value_type === 'rate_pct' ? 'Event Rate (%)' : 'Value');
      const syntheticEndpoint = {
        id:                    'synthetic_primary',
        label:                 parsed.library_meta?.display_title || 'Primary Outcome',
        chart_priority:        1,
        recommended_chart_type: 'bar', // index.html routes forest/bar via value_type, not this field
        clinical_synthesis:    pico.primary_outcome || '',
        axes: { x_label: 'Treatment Group', y_label: yLabel },
        arms: [
          { group_name: armAName, data_points: [{ x: armAName, y: selected.arm_a_value }] },
          { group_name: armBName, data_points: [{ x: armBName, y: selected.arm_b_value }] },
        ],
        value_type:      selected.value_type     ?? null,
        effect_measure:  selected.effect_measure ?? null,
        p_value:         selected.p_value        ?? null,
  ...
  let domain   = TAXONOMY_DOMAINS.includes(lm.domain) ? lm.domain : TAXONOMY_DOMAINS[0];
  let specs    = TAXONOMY_SPECIALTIES[domain] || [];
  let specialty = specs.includes(lm.specialty) ? lm.specialty : null;
  // Cross-domain rescue: if specialty matches another domain's list, correct the domain
  // (catches e.g. domain=Surgery specialty=Hip → corrects to Orthopaedics)
  if (!specialty && lm.specialty) {
    for (const [d, dSpecs] of Object.entries(TAXONOMY_SPECIALTIES)) {
      if (dSpecs.includes(lm.specialty)) {
        domain    = d;
        specs     = dSpecs;
  ...
  if (!specialty) specialty = specs[0] || null;
...
export async function runPipeline(sourceContext, sourceMeta = {}, adjMeta = {}, isEscalation = false) {
  ...
  const thinkingBudget    = isEscalation ? 8000 : 512; // flash-lite minimum is 512
  ...
  // Gemini gets a 5s head start before gpt-4o-mini fires.
  // If Gemini rate-limits it throws in ~2s (before the delay), so B is never called.
  // On a successful run, B starts 5s late but still finishes ~15s before Gemini,
  // so the parallel benefit is mostly preserved.
  let rawA, rawB;
  if (process.env.OPENAI_API_KEY) {
    const geminiPromise = callGemini(extractModel, EXTRACTOR_PROMPT_A + escalationWarning, sourceContext, { thinkingBudget });
    await new Promise(r => setTimeout(r, 5000));
    const openaiPromise = callOpenAI(EXTRACTOR_PROMPT_B + escalationWarning, sourceContext);
    [rawA, rawB] = await Promise.all([geminiPromise, openaiPromise]);
  } else {
    rawA = await callGemini(extractModel, EXTRACTOR_PROMPT_A + escalationWarning, sourceContext, { thinkingBudget });
    rawB = await callGemini(extractModel, EXTRACTOR_PROMPT_B + escalationWarning, sourceContext, { thinkingBudget });
  }
