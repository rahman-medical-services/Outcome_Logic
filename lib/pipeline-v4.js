...
// ─────────────────────────────────────────────
// BACK-CALCULATION — deterministic JS arithmetic
// Runs AFTER applyPatches(), BEFORE auditMetaAnalysisFields().
// Computes arm_a_events / arm_b_events for binary and time-to-event outcomes only.
// Uses arm_a_n (critic-patched) falling back to n_arm_a (V1 legacy field name).
// Continuous and ordinal outcomes: events are not applicable — leaves null.
// Does NOT overwrite existing non-null values (respects direct extraction).
// ─────────────────────────────────────────────
function backCalculateEvents(result) {
  ...
  cands.forEach(c => {
    ...
    // Back-calculate events only when arm_n and arm_value (percentage) are both present
...
function auditMetaAnalysisFields(result) {
  ...
  candidates.forEach((c, i) => {
    ...
    if (missing.includes('arm_a_sd') || missing.includes('arm_b_sd')) {
      diagnosis.push('SD per arm missing — requires human extraction (LLM SD patching disabled due to cross-variable contamination risk)');
    }
...
export async function runPipelineV4(sourceContext, sourceMeta = {}) {
  ...
  // Node 3b: Deterministic event back-calculation (JS arithmetic — no LLM)
  // Computes arm_a/b_events from arm_n × arm_value for binary/time-to-event outcomes.
  // Runs after patches so arm_a_n values patched by Rule 8 are available.
