...
// ─────────────────────────────────────────────
// REFERENCE STRIPPING
// Removes the References / Bibliography section to reduce critic input tokens.
// The reference list contains no extractable clinical data.
// Matches the last occurrence of the heading to avoid false matches in body text.
// ─────────────────────────────────────────────
function stripReferences(text) {
  ...
  if (lastMatch) {
    ...
    console.log(`[V4 Critic] Stripped reference section (saved ${saved} chars)`);
...
You have access to the paper text (Abstract, Methods, Results — References omitted).
Output STRICTLY the JSON object defined in the task. No preamble, no markdown fences, no text outside the JSON.`;

const CRITIC_TASK = `Review the extraction against the paper text. Perform two passes.
...
Check clinician_view.adverse_events.rows[*].event.
If any listed AE is a pre-specified trial endpoint (death, stroke, MI, repeat revascularisation,
AF, new LBBB, new pacemaker, valve dysfunction, renal failure — if pre-specified in this trial),
it must not appear in the AE table. Patch: set rows to the cleaned array, or set has_data to false
if the cleaned array is empty.
Path for rows: "clinician_view.adverse_events.rows"
...
These are required for downstream meta-analysis pooling.
- arm_a_n / arm_b_n null: patch to the per-arm sample sizes from Results or baseline table.
- arm_a_events / arm_b_events null for a binary outcome with event counts reported: patch to integer counts.
- effect_measure misclassified (e.g. "OR" for a time-to-event analysis that reported an HR): patch.
Paths: "primary_endpoint_candidates[0].arm_a_n", etc.
...
═══════════════════════════════════════════════════════
{
  "patches": [
    {
      ...
      "current_value": <the current value in the extraction — include for audit>,
  ...
  "violations_found": <integer — number of patches>
}`;
...
async function runCritic(draftJSON, paperText) {
  const stripped = stripReferences(paperText);
...
export async function runPipelineV4(sourceContext, sourceMeta = {}) {
  ...
  const v1Result = await runPipelineV1(sourceContext, sourceMeta);
  console.log('[V4 Pipeline] Node 1 complete. Starting Node 2 (gpt-4o-mini critic)');

  // Node 2: Critic review — paper text (sourceContext) minus References
  let criticResult;
  try {
    criticResult = await runCritic(v1Result, sourceContext);
  } catch (err) {
