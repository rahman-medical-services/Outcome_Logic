...
CRITICAL — MD/SMD GUARD (continuous outcomes):
For effect_measure ∈ {MD, SMD}, only patch arm_a_value or arm_b_value if the paper EXPLICITLY states
the per-arm change-from-baseline (or per-arm post-treatment mean) for THAT specific arm. Do NOT:
  - Use the between-arm mean difference itself as one arm's value while leaving the other at 0.
  - Set either arm to 0 unless the paper text explicitly states "no change" for that arm.
  - Fabricate one arm from the other plus the MD (e.g. arm_b = arm_a − MD).
If only the between-arm difference is reported (no per-arm change scores), leave BOTH arm_a_value
and arm_b_value as null. The pipeline will flag this as a meta-analysis gap.
...
For any candidate field where the paper contains genuinely conflicting or irresolvable values
(not merely a preferred vs alternative analysis — you should still choose the preferred one),
add an entry to uncertain_candidate_fields in your output.

WHEN to flag as uncertain:
  - Two sections of the paper report incompatible values for the same field with no clear authority
    (e.g. Results text says HR=0.82, Table 3 says HR=0.79, and neither is labelled as primary)
  - The paper is genuinely ambiguous about which arm is intervention vs control and both orderings
    are defensible (rare, but occurs in cross-over or factorial designs)
  - A field uses approximate language that makes an exact numeric value impossible
    (e.g. "approximately 50 patients per arm" — do not fabricate 50)

WHEN NOT to flag:
  - Randomised N vs analysed N exist but one is clearly preferred (use randomised N, no flag)
  - Adjusted vs unadjusted estimate (use adjusted, no flag)
  - Paper reports both OR and HR (use the designated primary, no flag — unless selection_uncertain applies)
  - A field is simply not reported (leave null, do NOT flag as uncertain)

Format: {"candidate_index": 0, "fields": ["field_name_1", "field_name_2"]}
═══════════════════════════════════════════════════════
...
═══════════════════════════════════════════════════════
{
  ...
  "violations_found": 0,
  "uncertain_candidate_fields": [
    { "candidate_index": 0, "fields": ["field_name"] }
  ]
}`;

// ─────────────────────────────────────────────
// LEGACY KEY CANONICALISATION — runs FIRST in post-processing
// V1 extractor uses legacy field names (n_arm_a, events_arm_a) that survived
// from the pre-V4 schema. V4 canonical names are arm_a_n, arm_a_events.
// Without this pass, downstream code reading canonical names sees null even
// when V1 extracted the data into the legacy field — producing silent data loss
// (confirmed: 40 candidates across 20 papers had n_arm_a populated with arm_a_n null).
//
// Runs BEFORE the critic so the critic sees clean canonical fields.
// Also handles two specific dirty inputs:
//   1. Compound fraction strings (SYNTAX has events_arm_a: "159/891")
//      → events to numerator, n to denominator if canonical n is null.
//   2. Literal string 'null' (ART has p_value: "null", events_arm_a: "null")
//      → coerced to real null.
// After copying, legacy keys are deleted to eliminate downstream ambiguity.
// ─────────────────────────────────────────────
function canonicaliseLegacyKeys(result) {
  const cands = result?.primary_endpoint_candidates;
  if (!Array.isArray(cands)) return;

  const keyMigrations = [
    { legacy: 'n_arm_a',      canonical: 'arm_a_n'      },
    { legacy: 'n_arm_b',      canonical: 'arm_b_n'      },
    { legacy: 'events_arm_a', canonical: 'arm_a_events' },
    { legacy: 'events_arm_b', canonical: 'arm_b_events' },
  ];

  cands.forEach(c => {
    // Step 1: Strip literal string 'null' from any field — treat as missing.
    // Also strip trailing '%' from arm value / SD fields (V1 sometimes emits "5.3%")
    // so that coerceNumericFields can parse them as numbers.
    Object.keys(c).forEach(k => {
      if (c[k] === 'null') { c[k] = null; return; }
      if (typeof c[k] === 'string' &&
          ['arm_a_value','arm_b_value','arm_a_sd','arm_b_sd'].includes(k) &&
          c[k].endsWith('%')) {
        c[k] = c[k].slice(0, -1).trim();
      }
    });

    // Step 2: Parse compound fractions like "159/891" in legacy events fields.
    // If both events and N are missing on canonical, populate both from the fraction.
    [['events_arm_a', 'arm_a_events', 'arm_a_n'],
     ['events_arm_b', 'arm_b_events', 'arm_b_n']].forEach(([legacy, canonEvents, canonN]) => {
      const v = c[legacy];
      if (typeof v === 'string' && /^\d+\/\d+$/.test(v)) {
        const [num, denom] = v.split('/').map(Number);
        if (c[canonEvents] == null && Number.isFinite(num))   c[canonEvents] = num;
        if (c[canonN]      == null && Number.isFinite(denom)) c[canonN]      = denom;
        delete c[legacy];
      }
    });

    // Step 3: Migrate legacy → canonical for each pair, then delete the legacy key.
    keyMigrations.forEach(({ legacy, canonical }) => {
      if (c[legacy] == null) {
        delete c[legacy];
        return;
      }
      if (c[canonical] == null) {
        c[canonical] = c[legacy];
      }
      delete c[legacy];
    });
  });
...
// ─────────────────────────────────────────────
// TYPE DISCIPLINE — final-boundary numeric coercion
// V1 LLM sometimes outputs numeric values as strings (e.g. arm_a_value: "32.5",
// n_arm_a: "602"). Without this pass, downstream meta-analysis consumers get
// inconsistent types: same field comes back as Number on one run and String on
// another. Confirmed across the 5×5 stability test:
//   TKR arm_a_value: 32.5 (number) × 3 vs '32.5' (string) × 2.
//
// Operates on all candidate-level numeric fields including arm_*_value and
// arm_*_sd (which the original coerceNumericFields missed).
// ─────────────────────────────────────────────
function coerceNumericFields(result) {
  ...
  const numericFields = [
    'arm_a_n', 'arm_b_n',
    'arm_a_events', 'arm_b_events',
    'arm_a_value', 'arm_b_value',
    'arm_a_sd', 'arm_b_sd',
    'value', 'point_estimate', 'ci_lower', 'ci_upper',
    'n_randomised_arm_a', 'n_randomised_arm_b', 'total_events',
  ];

  cands.forEach(c => {
    numericFields.forEach(f => {
      if (typeof c[f] === 'string' && c[f].trim() !== '' && !isNaN(c[f])) {
        c[f] = Number(c[f]);
      }
    });
  });
...
// ─────────────────────────────────────────────
// MD/SMD FABRICATION GUARD
// Detects the failure mode where, for a continuous (MD/SMD) outcome, the critic
// has set arm_a_value to the between-arm mean difference and arm_b_value to 0
// (or vice versa). This pattern fabricates a per-arm change score from the
// between-arm difference and is the most dangerous Rule 7 failure for
// meta-analysis: a precisely-wrong per-arm value that looks valid downstream.
//
// Pattern: effect_measure ∈ {MD, SMD} AND one arm value === 0 AND the other
//          arm value ≈ |value| (the between-arm difference within rounding).
// Action:  reset BOTH arm_a_value and arm_b_value to null and append a note to
//          meta_analysis_gaps so the audit trail records the intervention.
// ─────────────────────────────────────────────
function guardMDFabrication(result) {
  const cands = result?.primary_endpoint_candidates;
  if (!Array.isArray(cands)) return;

  cands.forEach((c, i) => {
    const isMD = ['MD', 'SMD'].includes(c.effect_measure);
    if (!isMD) return;
    const a = c.arm_a_value;
    const b = c.arm_b_value;
    const diff = c.value ?? c.point_estimate;
    if (a == null || b == null || diff == null) return;

    const aZero = a === 0;
    const bZero = b === 0;
    if (!aZero && !bZero) return;
    if (aZero && bZero) return; // both zero — paper may genuinely state this

    const nonZero = aZero ? b : a;
    // Match if non-zero arm equals |diff| within 5% tolerance (rounding/sign)
    const ratio = Math.abs(nonZero) > 0
      ? Math.abs(Math.abs(nonZero) - Math.abs(diff)) / Math.abs(diff || 1)
      : 1;
    if (ratio > 0.05) return;

    console.warn(`[V4 guardMDFabrication] candidate[${i}] "${c.label}": MD-fabrication pattern detected (arm_a=${a}, arm_b=${b}, diff=${diff}). Resetting per-arm values.`);
    c.arm_a_value = null;
    c.arm_b_value = null;
    c._md_fabrication_blocked = `arm values reset: pattern matched between-arm difference (${diff}) with one arm at 0 — per-arm change scores not extractable`;
  });
}
...
function backCalculateEvents(result) {
  ...
  cands.forEach(c => {
    ...
    // Legacy keys (n_arm_a, events_arm_a) have been migrated to canonical names
    // by canonicaliseLegacyKeys() upstream — read canonical only.
    const nA = c.arm_a_n;
    const nB = c.arm_b_n;

    // ARM A — back-calculate only when canonical events are still null
    ...
    } else if (c.arm_a_events != null && c._arm_a_events_source == null) {
      c._arm_a_events_source = 'extracted';
    }

    // ARM B — back-calculate only when canonical events are still null
    ...
    } else if (c.arm_b_events != null && c._arm_b_events_source == null) {
...
function backCalculateSD(result) {
  ...
  cands.forEach(c => {
    ...
    // Legacy keys migrated upstream by canonicaliseLegacyKeys — canonical only.
    const nA  = c.arm_a_n;
    const nB  = c.arm_b_n;
...
// ─────────────────────────────────────────────
// OUTCOME TYPE ENFORCEMENT FOR HR CANDIDATES
// HR is definitionally a time-to-event measure. No LLM vote can change this.
// Confirmed regression: ISCHEMIA V1 correctly had time_to_event; critic Rule 12
// downgraded it to "binary" reasoning "composite of events is binary". This is
// wrong: HR always implies a survival/competing-risk model, regardless of composite
// structure. Runs AFTER normaliseOutcomeTypes() and after all patches are applied.
// ─────────────────────────────────────────────
function enforceOutcomeTypeForRatioMeasures(result) {
  const cands = result?.primary_endpoint_candidates;
  if (!Array.isArray(cands)) return;

  cands.forEach((c, i) => {
    if (c.effect_measure === 'HR' && c.outcome_type !== 'time_to_event') {
      const prev = c.outcome_type;
      c.outcome_type = 'time_to_event';
      c._outcome_type_source = 'enforced:HR_always_time_to_event';
      console.warn(`[V4 enforceOutcomeType] candidate[${i}] "${c.label}": outcome_type ${JSON.stringify(prev)} → time_to_event`);
    }
  });
}

// ─────────────────────────────────────────────
// UNCERTAIN FIELDS MERGE
// Both V1 and the critic can flag fields as genuinely uncertain — where the paper
// contains conflicting or irresolvable information, not simply a missing value.
// The critic returns uncertain_candidate_fields: [{candidate_index, fields}].
// This function merges those flags onto the candidate objects (deduped).
// V1-populated uncertain_fields arrays (from V1 prompt output) are preserved as-is.
// ─────────────────────────────────────────────
function mergeUncertainFields(result, uncertainCandidateFields) {
  if (!Array.isArray(uncertainCandidateFields) || !uncertainCandidateFields.length) return;
  const cands = result?.primary_endpoint_candidates;
  if (!Array.isArray(cands)) return;

  for (const entry of uncertainCandidateFields) {
    const { candidate_index, fields } = entry;
    if (typeof candidate_index !== 'number' || !Array.isArray(fields)) continue;
    const c = cands[candidate_index];
    if (!c) continue;
    if (!Array.isArray(c.uncertain_fields)) c.uncertain_fields = [];
    for (const f of fields) {
      if (typeof f === 'string' && !c.uncertain_fields.includes(f)) {
        c.uncertain_fields.push(f);
      }
    }
    if (c.uncertain_fields.length) {
      console.log(`[V4 mergeUncertainFields] candidate[${candidate_index}] "${c.label}": uncertain_fields = [${c.uncertain_fields.join(', ')}]`);
    }
  }
}
...
function applyPatches(obj, patches) {
  const applied       = []; // substantive: value actually changed
  const verifications = []; // no-op: critic confirmed an already-correct value
  const skipped       = []; // guard-blocked or path error
  ...
  for (const patch of patches) {
    try {
      ...
      let cursor = obj;
      // Track candidate parent (for provenance tagging) when path is
      // primary_endpoint_candidates[N].<field>.
      ...
      for (let i = 0; i < parts.length - 1; i++) {
        ...
        cursor = cursor[key];
        if (parts[0] === 'primary_endpoint_candidates' &&
            i === 1 && typeof parts[1] === 'number') {
      ...
      // CI field guard: Rule 1 (CI completeness) should ONLY complete null CIs — never replace
      // existing ones. The prompt null-guard fires unreliably when the paper contains multiple
      // CI values on different scales. Confirmed failure: SYNTAX RR ci_upper (correct: 1.81)
      // overwritten by critic with absolute risk difference CI upper (8.3 percentage points).
      // The prompt says "only patch if null" but the critic ignored it. JS guard is the backstop.
      if ((patch.path.endsWith('.ci_lower') || patch.path.endsWith('.ci_upper')) &&
          before !== null && before !== undefined) {
        skipped.push({ path: patch.path, rule: patch.rule,
          reason: `CI-guard: cannot overwrite non-null CI (${JSON.stringify(before)}) — Rule 1 completes missing CIs only` });
        console.warn(`[V4 Critic] Patch blocked by CI-guard: ${patch.path} current=${JSON.stringify(before)}`);
        continue;
      }
      ...
      // Detect no-op (verification) vs substantive change.
      // A patch is substantive if the JSON serialisation of before/after differs.
      const isNoOp = JSON.stringify(before) === JSON.stringify(patch.corrected_value);
      ...
      if (isNoOp) {
        ...
        console.log(`[V4 Critic] Patch verified no-op: ${patch.path} [${patch.rule}]`);
      } else {
        applied.push(record);
        // Provenance tag for substantive candidate-field patches.
        // When path is primary_endpoint_candidates[N].<field>, set sibling
        // _<field>_source = "critic_patched:<rule>" so downstream consumers can
        // distinguish V1-extracted values from critic-corrected values.
        if (candidateParent && typeof lastKey === 'string' && !lastKey.startsWith('_')) {
          candidateParent[`_${lastKey}_source`] = `critic_patched:${patch.rule || 'unknown'}`;
        }
  ...
  return { applied, verifications, skipped };
}
...
async function runCritic(draftJSON, paperText) {
  ...
  if (!Array.isArray(criticResult.uncertain_candidate_fields)) criticResult.uncertain_candidate_fields = [];
...
function auditMetaAnalysisFields(result) {
  ...
  candidates.forEach((c, i) => {
    const missing   = [];
    const available = [];
    const uncertainFields = Array.isArray(c.uncertain_fields) ? c.uncertain_fields : [];
    ...
    if (missing.includes('arm_a_sd') || missing.includes('arm_b_sd')) {
      ...
      const hasN  = c.arm_a_n != null && c.arm_b_n != null;
      if (hasCI && hasN) {
    ...
    // Classify each missing field: genuinely uncertain (flagged) vs simply absent
    const uncertain = missing.filter(f => uncertainFields.includes(f));
    const absent    = missing.filter(f => !uncertainFields.includes(f));

    gaps.push({
      candidate:   `candidates[${i}] — ${c.label || 'unlabelled'}`,
      outcome_type: c.outcome_type || null,
      missing,         // all null fields (uncertain + absent combined — for backwards compat)
      uncertain,       // null because paper has conflicting/irresolvable values
      absent,          // null because simply not reported in the paper
      available,
      diagnosis,
      context: {
...
export async function runPipelineV4(sourceContext, sourceMeta = {}) {
  ...
  // Canonicalise legacy V1 field names (n_arm_a→arm_a_n, events_arm_a→arm_a_events,
  // compound fractions, string 'null' → null) BEFORE critic call so critic reads
  // clean canonical fields. Without this pass, ~40 candidates across 20 papers
  // had stranded legacy data that downstream canonical-key reads missed.
  canonicaliseLegacyKeys(v1Result);
  ...
  // applied        = substantive value changes
  // verifications  = no-ops (critic re-stated an already-correct value)
  // skipped        = guard-blocked or path-resolution failures
  console.log('[V4 Pipeline] Node 3: applying patches');
  const { applied, verifications, skipped } = applyPatches(v1Result, criticResult.patches);
  ...
  // Node 3b-bis: Merge uncertain_fields flags from critic onto candidates.
  // The critic returns uncertain_candidate_fields: [{candidate_index, fields}] for any field
  // where it found genuinely irresolvable ambiguity (conflicting values, not just missing data).
  // Deduped merge with any uncertain_fields already set by V1 extractor.
  mergeUncertainFields(v1Result, criticResult.uncertain_candidate_fields);
  ...
  // Node 3c-bis: MD/SMD fabrication guard — detects the pattern where Rule 7
  // patched one arm to the between-arm difference and the other arm to 0.
  // Resets both per-arm values to null so downstream consumers do not pick up
  // a fabricated per-arm change score.
  guardMDFabrication(v1Result);
  ...
  // Node 3e-bis: Enforce outcome_type for HR candidates.
  // HR is definitionally time_to_event — no LLM vote overrides this.
  // Catches regressions where critic Rule 12 reclassifies HR candidates as "binary"
  // (confirmed: ISCHEMIA V1 correct, critic changed to binary; CORONARY V1 wrong, critic missed).
  enforceOutcomeTypeForRatioMeasures(v1Result);
  ...
  v1Result._critic = {
    patches_applied:      applied.length,        // substantive only
    verifications_count:  verifications.length,  // no-ops (already-correct re-statements)
  ...
  console.log(`[V4 Pipeline] Complete. ${applied.length} substantive patches, ${verifications.length} verifications, ${skipped.length} skipped, ${criticResult.quality_notes.length} quality notes.`);
