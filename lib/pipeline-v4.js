...
// Runs AFTER backCalculateEvents(), BEFORE auditMetaAnalysisFields().
//
// For continuous outcomes where arm_a_sd is missing but CI and arm N are both present:
//   SE  = (ci_upper − ci_lower) / (2 × 1.96)
//   pooled_SD = SE / √(1/nA + 1/nB)
//
// Sets BOTH arm_a_sd and arm_b_sd to the pooled SD (they are equal by this method).
// Tags with _sd_source = 'back-calculated from CI and N (Cochrane §6.5.2)'.
// Does NOT overwrite existing non-null SD values (direct extraction takes precedence).
// ─────────────────────────────────────────────
function backCalculateSD(result) {
  ...
  cands.forEach(c => {
    ...
    // Do not overwrite directly-extracted SD
    if (c.arm_a_sd != null) return;
...
export async function runPipelineV4(sourceContext, sourceMeta = {}) {
  ...
  // Node 3b: Coerce string integers to Number (e.g. n_arm_a: "602" → 602).
  ...
  // SE = (CI_upper − CI_lower) / (2 × 1.96), pooled_SD = SE / √(1/nA + 1/nB).
  // Only fires when arm_a_sd is null and both CI and N are present.
