...
async function _extractIdentityFromReport(reportA) {
  ...
  try {
    ...
    const prompt = `From this partial clinical trial extraction, return ONLY this JSON — no preamble, no markdown:
{
  "trial_name": "full name and acronym if present, else null",
  "first_author_surname": "surname only, else null",
  "year": "4-digit year, else null",
  "pmid": "if explicitly mentioned in the text, else null",
  "doi": "if explicitly mentioned in the text, else null"
}
...
async function _resolvePmidFromIdentity(identity) {
  try {
    const parts = [];
    if (identity.trial_name)          parts.push(`${identity.trial_name}[TITLE]`);
    if (identity.first_author_surname) parts.push(`${identity.first_author_surname}[AUTH]`);
    if (identity.year)                 parts.push(`${identity.year}[PDAT]`);
    ...
    // Confidence check: at least one title keyword should appear in result title
    const titleWords = (identity.trial_name || '').toLowerCase().split(/\s+/).filter(w => w.length > 3);
