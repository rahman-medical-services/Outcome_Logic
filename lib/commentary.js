...
const MIN_ITEMS_FOR_SYNTHESIS = 3;  // need at least this many qualifying items — 2 is insufficient for synthesis
...
async function callGemini(model, systemInstruction, userContent, options = {}) {
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    if (attempt > 0) {
      ...
      console.log(`[Node 4 callGemini] Retry ${attempt}/${MAX_RETRIES - 1} after ${Math.round(delay)}ms`);
    }
    try {
...
async function _runCommentaryFlow(sourceMeta, reportA) {
  ...
  // Promise.allSettled — one API hang should not discard results from the others.
  const [citationsResult, nameResult, pubmedResult] = await Promise.allSettled([
  ...
  const { hitCount, citations } = citationsResult.status === 'fulfilled'
    ? citationsResult.value
    : (console.warn('[Node 4] Citations fetch rejected:', citationsResult.reason?.message), { hitCount: 0, citations: [] });
  const nameResults   = nameResult.status   === 'fulfilled' ? nameResult.value   : [];
  const pubmedResults = pubmedResult.status === 'fulfilled' ? pubmedResult.value : [];
  ...
  // Fetch abstracts — allSettled so a single slow/failing fetch doesn't drop the batch.
  const abstractResults = await Promise.allSettled(
    scored.map(c => _fetchAbstract(c.id || c.pmid).then(abstract => ({ ...c, abstract })))
  );
  const withAbstracts = abstractResults
    .map((r, i) => r.status === 'fulfilled' ? r.value : { ...scored[i], abstract: null });
...
- If abstract unavailable, classify from title. "Invited Commentary" = critique. "Comment"/"Letter" = critique.`;
    ...
    const text = await callGemini(
      GEMINI_MODEL,
      'You are an expert clinical appraiser. Categorise each citing paper. Always respond with valid JSON only. No preamble, no explanation, no markdown fences.',
      prompt,
...
If you cannot find reliable information specifically about this trial, write: "Post-publication commentary for this trial is not yet available in indexed sources."`;
    ...
    const synthesis = (await callGemini(
      GEMINI_MODEL,
      `You are an expert clinical academic writing a concise evidence context paragraph for a clinical trial database. You have access to Google Search — use it to find post-publication commentary, critiques, meta-analyses, and guideline citations for the trial. CRITICAL RULES: Search for the trial by name and full title to find real published responses. Only state what you can verify from your search results. Attribute specific criticisms or endorsements to named authors and their publications. Do NOT restate what the trial itself found — focus on how others have responded to it. Do NOT hallucinate statistics, author names, or journal names. Write in plain clinical English, past tense, 3-5 sentences. If search results are sparse, write fewer sentences rather than speculating.`,
      prompt,
...
async function _runSynthesis(itemsWithAbstracts, trialName, trialTitle) {
  // allSettled — web synthesis failing (e.g. googleSearch quota) should not discard item categorisation.
  const [itemsResult, synthesisResult] = await Promise.allSettled([
