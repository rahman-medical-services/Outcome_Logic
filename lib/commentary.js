...
function _extractIdentityFromReport(reportA) {
  ...
  try {
    ...
    // ── Trial name — look for common extractor output patterns ─────────────
    // Extractor writes: "Full trial name and acronym: XXXX"
    // or "Trial: XXXX" or just the acronym in caps early in output
    let trial_name = null;
    const namePatterns = [
      /(?:full\s+trial\s+name(?:\s+and\s+acronym)?|trial\s+name|trial\s+identification)\s*[:\-]\s*([^\n]{3,80})/i,
      /(?:acronym|trial)\s*[:\-]\s*([A-Z][A-Z0-9\-]{1,20}(?:\s+trial)?)/,
    ];
    for (const re of namePatterns) {
      const m = text.match(re);
      if (m?.[1]?.trim()) { trial_name = m[1].trim().replace(/\.$/, ''); break; }
    }

    // ── First author surname ────────────────────────────────────────────────
    // Extractor writes: "Authors: Surname INITIALS, Surname INITIALS et al."
    let first_author_surname = null;
    const authorMatch = text.match(/(?:Authors?)\s*[:\-]\s*([A-Z][a-züöäßé\-']{1,30})/);
    if (authorMatch?.[1]) first_author_surname = authorMatch[1].trim();

    // ── Year ────────────────────────────────────────────────────────────────
    // Look for "Year: YYYY" or "YYYY" near journal/publication context
    let year = null;
    const yearMatch = text.match(/(?:year|published|publication)\s*[:\-]?\s*(\b20[0-2]\d\b)/i)
                   || text.match(/\b(20[0-2]\d)\b/);
    if (yearMatch?.[1]) year = yearMatch[1];

    // Return null if nothing useful
    if (!trial_name && !pmid && !doi) {
      console.log('[Node 4] ID extraction: nothing found in reportA');
      return null;
    }

    console.log(`[Node 4] ID extraction: trial="${trial_name}", author="${first_author_surname}", year="${year}", pmid="${pmid}", doi="${doi}"`);
    return { trial_name, first_author_surname, year, pmid, doi };
