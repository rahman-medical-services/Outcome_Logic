...
//
// Gemini calls use raw fetch() with thinkingBudget: 512 and retry on 503.
// See lib/pipeline.js header for full rationale on SDK removal and model choice.

const GEMINI_API_KEY = process.env.GEMINI_API_KEY;
const GEMINI_BASE    = 'https://generativelanguage.googleapis.com/v1beta/models';
const GEMINI_MODEL   = 'gemini-2.5-flash-lite';

async function callGemini(model, { systemInstruction, contents, temperature = 0, tools } = {}) {
  const url  = `${GEMINI_BASE}/${model}:generateContent?key=${GEMINI_API_KEY}`;
  const body = { contents, generationConfig: { temperature, thinkingConfig: { thinkingBudget: 512 } } };
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (res.status === 503 && attempt < MAX_RETRIES) {
      ...
      console.warn(`[Node 4 Gemini] 503 on attempt ${attempt}/${MAX_RETRIES} — retrying in ${delay / 1000}s`);
...
If you cannot find reliable information specifically about this trial, write: "Post-publication commentary for this trial is not yet available in indexed sources."`;
    ...
    const synthesis = (await callGemini(GEMINI_MODEL, {
