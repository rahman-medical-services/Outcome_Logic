...
function _extractIdentityFromReport(reportA) {
  ...
  try {
    ...
    const namePatterns = [
      ...
      /\b([A-Z][A-Z0-9\-]{2,15}(?:\s+trial)?)\b/,   // uppercase acronym as last resort
    ];
    for (const re of namePatterns) {
      const m = text.match(re);
      const candidate = m?.[1]?.trim().replace(/\.$/, '');
      // Skip if candidate is a common false-positive word
      if (candidate && !['NOT', 'NULL', 'NONE', 'UNKNOWN'].includes(candidate.toUpperCase())) {
